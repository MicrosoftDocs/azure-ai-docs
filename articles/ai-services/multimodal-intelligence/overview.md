...
Azure AI Multimodal Intelligence is a cloud-based solution on the Azure platform designed to process various data types such as documents, images, videos, and audio within a unified workflow. Integrating these diverse data types enables Multimodal Intelligence to deliver more extensive, efficient, and accurate results compared to single modality processing. Utilizing the Multimodal Intelligence service, provides you with access to the following processes:
...
|Grounded results for higher accuracy|A grounded response adheres strictly to the provided information, avoiding any speculation or fabrication. The source information is pivotal to assessing groundedness, serving as the foundation for both grounding and accuracy. Multimodal Intelligence ensures that responses are anchored to your input files, leading to higher precision in extracted values.|
|Enhanced data-input alignment framework leading to highly precise confidence scores|A confidence score is a number between `0` and `1` that indicates probability by measuring the degree of statistical certainty that the extracted result accurate, correct, and reliable. High confidence scores are an indicator of data extraction accuracy. High extraction accuracy scores support straight through processing (STP) for automation workflows.|
|Precise and efficient extraction of modality-specific details for secondary processing scenarios| Users can extract specific content suitable for secondary scenarios such as Large Language Model (LLM) processing.|
