...
- Access to the gated artifacts required for the Azure Vision in Foundry Tools Face Client SDK for Mobile (IOS and Android) and Web. 
...
1. The SDK calls the Azure Vision Face service to perform the liveness detection. Once the service responds, the SDK notifies the frontend application that the liveness check has been completed. Note: the service response will not contain the liveness decision, and this will need to be queried from the app server.

1. The frontend application relays the liveness check completion to the app server. 

1. The app server can now query for the liveness detection result from the Azure Vision Face service. 
