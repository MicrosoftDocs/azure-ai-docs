...
When you send asynchronous requests, you incur charges based on number of text records you include in your request, for each feature use. For example, if you send a text record for sentiment analysis and NER, it is counted as sending two text records, and you're charged for both according to your [pricing tier](https://azure.microsoft.com/pricing/details/cognitive-services/language-service/). 

## Submit an asynchronous job using the REST API

To submit an asynchronous job, review the [reference documentation](/rest/api/language/analyze-text-submit-job) for the JSON body you send in your request.
...
A successful call will return a 202 response code. The `operation-location` in the response header is the URL you use to retrieve the API results. The value will look similar to the following URL:
