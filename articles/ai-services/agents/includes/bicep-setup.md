...
| Deploy a standard agent setup with private networking that uses User Managed Identity for authentication. <br><br> Resources for the AI hub, AI project, key vault, storage account, AI Services, and AI Search are created for you. <br><br> The AI Services, AI Search, key vault, and storage account are connected to your project and hub, and a gpt-4o-mini model is deployed in the westus2 region. Customer-owned, single-tenant search and storage resources are provisioned in a secured, managed network environment—with private links and DNS zones configured on your behalf—and authenticated via User Managed Identity with the necessary RBAC permissions. Note that usage-based costs apply for the customer-owned resources. | [See more details](../how-to/use-your-own-vnet.md) |
