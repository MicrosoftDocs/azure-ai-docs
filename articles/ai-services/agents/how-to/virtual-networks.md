...
1. Using the resource group you created in the previous step and one of the template files (`network-secured-agent`), run one of the following commands: 
    ...
    1. To specify custom names for the hub, project, storage account, or other Azure AI services resources, run the following command. A randomly generated suffix is added to prevent accidental duplication.
