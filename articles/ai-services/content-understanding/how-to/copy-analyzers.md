...
All prebuilt analyzers in Content Understanding are available in every resource by default. Find the complete list of all [prebuilt analyzers here](). Custom analyzers are analyzers you define to process a specifc content where you define the content type, schema and any other processing logic. For more information on defining a custom analyzer see [defining a custom analyzer]().

The copy operation on analyzers supports a few scenarios:
* **Copy within resource** to create a copy of an existing analyzer in the same resource as a backup or a version you can iteratlively make changes from. 
* **Copy across resources** copy an analyzer from one AI Foundry resource to another. This support failover scenarios and sharing of analyzers across teams.
...
Copying the analyzer across AI Foundry resources is a multi-step process as it is assumed a service principal will not have permissions on both resources:
...
```
 ...
 > [!NOTE]
>
> Analyzers now support classification/segmentation and analysis of each of the identified classes/segments in a single request. When copying an analyzer that uses this feature, you will need to copy any analyzers referenced as well.
