...
* **Identity verification**: Process passports, driver's licenses, ID cards, health insurance cards, and identity documents from multiple countries with `prebuilt-idDocument` and related analyzers. Extract personal information, document numbers, and verification details with support for worldwide formats. See the [identity documents](#identity-documents) section below.
