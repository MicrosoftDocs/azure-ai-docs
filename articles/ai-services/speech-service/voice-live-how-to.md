...
| `prefix_padding_ms` | integer | Optional  | The amount of audio, measured in milliseconds, to include before the start of speech detection signal (default: 400 for `server_vad`, 420 for `azure_semantic_vad` and `azure_semantic_vad_multilingual`). |
