...
- Photo Avatar: You create the avatar from a single input image as prompt and is limited to a head-only representation. Photo Avatar is available in two variants: standard Photo Avatar, which generates a talking-head video from a single photo, and custom Photo Avatar, which lets you fine-tune the avatar's look from your own image. For more information about the custom variant, see [What is custom text to speech avatar](./what-is-custom-text-to-speech-avatar.md).
...
You can create custom text to speech avatars that are unique to your product or brand. For a custom video avatar, all it takes to get started is 10 minutes of video recordings. For a custom photo avatar, you need a photo along with about one minute of consent audio, which the service uses to better match the voice to the avatar. If you fine-tune a professional voice for the actor, the avatar can be highly realistic. 
