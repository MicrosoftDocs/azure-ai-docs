...
## Avatar type
- Video Avatar: The avatar is generated using a neural network model and requires a video recording for training. It supports half-body and full-body representations.
- Photo Avatar: The avatar is created using the Vasa-1 model from a single input image and is limited to a head-only representation.
