...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="188-215,417-475" highlight="9,27,35-38":::
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="188-215,417-475" highlight="14-18,29-30,37-38":::
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="361-393" highlight="3-32":::
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="259-287" highlight="6-15,21":::
...
The sample doesn't configure `auto_truncate` in `turn_detection`.
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="299-319":::
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="426,451-454":::

Conversation reconnect is applied in three places:

- In `Main()`, read `CONVERSATION_ID` from the environment (line 426).
- Pass the value to the `BasicVoiceAssistant(...)` constructor (lines 451-454).
...
:::code language="csharp" source="~/voice-live-samples-code/csharp/voice-live-quickstarts/AgentsNewQuickstart/VoiceLiveWithAgentV2.cs" range="186-187,299-319,395-407" highlight="1-2,7-8,28-36":::

Session metadata logging is applied in three places:

- A timestamped conversation log file (`conversation_YYYYMMDD_HHmmss.log`) is created per run (lines 186–187).
- On `SessionUpdateSessionUpdated`, the handler extracts the session ID and writes it to the log (lines 307–308).
- `WriteLog(...)` appends entries throughout the conversation lifecycle (lines 395–407).
