...
```HTTP
GET https://{endpoint}/openai/vector_stores/{vector_store_id}?api-version=2024-09-01-preview
```
...
```HTTP
POST https://{endpoint}/openai/vector_stores/{vector_store_id}?api-version=2024-09-01-preview
```
...
```HTTP
DELETE https://{endpoint}/openai/vector_stores/{vector_store_id}?api-version=2024-09-01-preview
```
...
```HTTP
GET https://{endpoint}/openai/vector_stores/{vector_store_id}/files?api-version=2024-09-01-preview
```
...
```HTTP
POST https://{endpoint}/openai/vector_stores/{vector_store_id}/files?api-version=2024-09-01-preview
```
...
```HTTP
GET https://{endpoint}/openai/vector_stores/{vector_store_id}/files/{file_id}?api-version=2024-09-01-preview
```
...
```HTTP
DELETE https://{endpoint}/openai/vector_stores/{vector_store_id}/files/{file_id}?api-version=2024-09-01-preview
```
...
```HTTP
POST https://{endpoint}/openai/vector_stores/{vector_store_id}/file_batches?api-version=2024-09-01-preview
```
...
```HTTP
GET https://{endpoint}/openai/vector_stores/{vector_store_id}/file_batches/{batch_id}?api-version=2024-09-01-preview
```
...
```HTTP
POST https://{endpoint}/openai/vector_stores/{vector_store_id}/file_batches/{batch_id}/cancel?api-version=2024-09-01-preview
```
...
```HTTP
GET https://{endpoint}/openai/vector_stores/{vector_store_id}/file_batches/{batch_id}/files?api-version=2024-09-01-preview
```
...
| max_tokens | integer | The maximum number of tokens allowed for the generated answer. By default, the number of tokens the model can return will be (4096 - prompt tokens). This value is now deprecated in favor of `max_completion_tokens`, and is not compatible with o1 series models. | No | 4096 |
| max_completion_tokens | integer | An upper bound for the number of tokens that can be generated for a completion, including visible output tokens and reasoning tokens. | No |  |
...
| Name | Type | Description | Default |
|------|------|-------------|--------|
| detail | string | Specifies the detail level of the image. Learn more in the [Vision guide](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/gpt-with-vision?tabs=rest%2Csystem-assigned%2Cresource#detail-parameter-settings-in-image-processing-low-high-auto). | auto |
