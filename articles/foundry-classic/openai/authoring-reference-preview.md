---
title: "Azure OpenAI in Microsoft Foundry Models REST API authoring preview reference (classic)"
description: "Learn how to use Azure OpenAI's latest authoring preview REST API. In this article, you learn about authorization options,  how to structure a request and receive a response. (classic)"
manager: nitinme
ms.service: azure-ai-foundry
ms.subservice: azure-ai-foundry-openai
ms.topic: reference
ms.date: 12/6/2025
author: mrbullwinkle
ms.author: mbullwin
recommendations: false
ms.custom:
- classic-and-new
ROBOTS: NOINDEX, NOFOLLOW
---

# Azure OpenAI in Microsoft Foundry Models authoring REST API preview reference (classic)

**Currently viewing:** :::image type="icon" source="../../foundry/media/yes-icon.svg" border="false"::: **Foundry (classic) portal version** - [Switch to version for the new Foundry portal](../../foundry/openai/authoring-reference-preview.md)

[!INCLUDE [authoring-reference-preview content](../../foundry/openai/includes/authoring-reference-preview-content.md)]
