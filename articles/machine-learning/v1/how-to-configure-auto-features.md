...
ms.topic: how-to
ms.custom: UpdateFrequency5, automl, sdkv1, dev-focus
...
Training data consists of rows and columns. Each row is an observation or record, and the columns of each row are the features that describe each record. Typically, you select the features that best characterize the patterns in the data to create predictive models.

Although you can use many of the raw data fields directly to train a model, you often need to create other (engineered) features that provide information to better differentiate patterns in the data. This process is called *feature engineering*. Use domain knowledge of the data to create features that help machine learning algorithms learn better.
...
Every AutoML experiment automatically applies [automatic scaling and normalization techniques](#featurization) to your data. These techniques are types of featurization that help *certain* algorithms that are sensitive to features on different scales. You can enable more featurization, such as *missing-values imputation*, *encoding*, and *transforms*.
...
------------- | ------------- |
|`"featurization": 'auto'`| Specifies that, as part of preprocessing, [data guardrails](#data-guardrails) and [featurization steps](#featurization) are done automatically. This setting is the default.|
|`"featurization": 'off'`| Specifies that featurization steps aren't done automatically.|
|`"featurization": 'FeaturizationConfig'`| Specifies that customized featurization steps are used. To learn more, see [Customize featurization](#customize-featurization).|

<a name="featurization"></a>

## Automatic featurization

The following table summarizes techniques that the system automatically applies to your data. These techniques apply to experiments that you configure by using the SDK or the studio UI. To disable this behavior, set `"featurization": 'off'` in your `AutoMLConfig` object.
...
|**Impute missing values*** |For numeric features, imputes with the average of values in the column.<br><br>For categorical features, imputes with the most frequent value.|
...
|**Transform and encode***|Transforms numeric features that have few unique values into categorical features.<br><br>*One-hot encoding* is used for low-cardinality categorical features. *One-hot-hash encoding* is used for high-cardinality categorical features.|
...
In every AutoML experiment, the system automatically scales or normalizes your data to help algorithms perform well. During model training, the system applies one of the following scaling or normalization techniques to each model.
...
*Data guardrails* help you identify potential problems with your data, such as missing values or [imbalanced data](../concept-manage-ml-pitfalls.md#identify-models-with-imbalanced-data). They also help you take corrective actions for better results.

Apply data guardrails in the following ways:

- **For SDK experiments**: Specify the parameters `"featurization": 'auto'` or `validation=auto` in your `AutoMLConfig` object.
- **For studio experiments**: Enable automatic featurization.
...
|State| Description |
|----|---- |
|**Passed**| No data problems were detected and no action is required. |
|**Done**| Changes were applied to your data. Review the corrective actions that AutoML took to ensure that the changes align with the expected results. |
|**Alerted**| A data problem was detected but couldn't be fixed. Revise and fix the problem.|
...
|**Cross validation** |Done| To accurately evaluate the model trained by AutoML, use a dataset that the model isn't trained on. If you don't provide an explicit validation dataset, a part of the training dataset is used to achieve this. For smaller datasets (fewer than 20,000 samples), AutoML uses cross-validation. For larger datasets, AutoML uses a single hold-out set split from the training data to serve as the validation dataset. For your input data, AutoML uses cross-validation with 10 folds if the number of training samples is fewer than 1,000, and three folds in all other cases.|
|**Train-test data split** |Done| To accurately evaluate the model trained by AutoML, use a dataset that the model isn't trained on. If you don't provide an explicit validation dataset, a part of the training dataset is used to achieve this. For smaller datasets (fewer than 20,000 samples), AutoML uses cross-validation. For larger datasets, AutoML uses a single hold-out set split from the training data to serve as the validation dataset. Hence, your input data is split into a training dataset and a holdout validation dataset.|
...
|**Short series handling** |Passed <br><br><br> Fixed |AutoML detected enough data points for each series in the input data to continue with training. <br> <br> AutoML detected that some series didn't contain enough data points to train a model. To continue with training, AutoML drops or pads these short series.|

## Customize featurization

Customize your featurization settings to make sure the data and features used to train your machine learning model lead to relevant predictions.
...
|**Block transformers**| Specifies block transformers to use in the featurization process.|
...
Every AutoML model automatically applies featurization. Featurization includes automated feature engineering (when `"featurization": 'auto'`) and scaling and normalization. These steps affect the selected algorithm and its hyperparameter values. AutoML provides different methods to help you see what featurization it applied to your model.

Consider this forecasting example:

+ The dataset has four input features: A (Numeric), B (Numeric), C (Numeric), and D (DateTime).
+ The process drops numeric feature C because it's an ID column with all unique values.
+ The process imputes missing values in numeric features A and B by using the mean.
+ The process featurizes DateTime feature D into 11 different engineered features.
...
The `get_engineered_feature_names()` function returns a list of engineered feature names.

>[!Note]
>Use `'timeseriestransformer'` for `task='forecasting'`. For `'regression'` or `'classification'` tasks, use `'datatransformer'`.
...
The `get_featurization_summary()` function gets a featurization summary of all the input features.
...
|Output|Definition|
|----|--------|
|RawFeatureName|Input feature or column name from the dataset provided.|
...
To understand scaling and normalization, and to see the selected algorithm with its hyperparameter values, use `fitted_model.steps`.

The following sample output comes from running `fitted_model.steps` for a chosen run:
...
This helper function returns the following output for a particular run that uses `LogisticRegression with RobustScalar` as the specific algorithm.
...
Models that AutoML creates all have wrapper objects that mirror functionality from their open-source origin class. Most classification model wrapper objects that AutoML returns implement the `predict_proba()` function. This function accepts an array-like or sparse matrix data sample of your features (X values), and returns an n-dimensional array of each sample and its respective class probability.

If you retrieve the best run and fitted model by using the same calls, you can call `predict_proba()` directly from the fitted model. Supply an `X_test` sample in the appropriate format depending on the model type.
...
If the underlying model doesn't support the `predict_proba()` function or the format is incorrect, the code throws a model class-specific exception. For examples of how this function is implemented for different model types, see the [RandomForestClassifier](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html#sklearn.ensemble.RandomForestClassifier.predict_proba) and [XGBoost](https://xgboost.readthedocs.io/en/latest/python/python_api.html) reference docs.
...
To invoke BERT, set `enable_dnn: True` in your `automl_settings` and use a GPU compute (`vm_size = "STANDARD_NC6"` or higher GPU). If you use a CPU compute, AutoML enables the BiLSTM DNN featurizer instead of BERT.

AutoML takes the following steps for BERT:

1. **Preprocesses and tokenizes all text columns**. For example, the `StringCast` transformer can be found in the final model's featurization summary. You can find an example of how to produce the model's featurization summary in this [Jupyter notebook](https://github.com/Azure/azureml-examples/blob/v1-archive/v1/python-sdk/tutorials/automl-with-azureml/classification-text-dnn/auto-ml-classification-text-dnn.ipynb).

1. **Concatenates all text columns into a single text column**, hence the `StringConcatTransformer` in the final model.

    Our implementation of BERT limits total text length of a training sample to 128 tokens. That means, all text columns when concatenated, should ideally be at most 128 tokens in length. If multiple columns are present, each column should be pruned so this condition is satisfied. Otherwise, for concatenated columns of length more than 128 tokens, BERT's tokenizer layer truncates this input to 128 tokens.
...
BERT generally runs longer than other featurizers. For better performance, use *STANDARD_NC24r* or *STANDARD_NC24rs_V3* for their RDMA capabilities.

AutoML distributes BERT training across multiple nodes if they're available (up to a max of eight nodes). You can do this in your `AutoMLConfig` object by setting the `max_concurrent_iterations` parameter to higher than 1.

## Supported languages for BERT

AutoML currently supports around 100 languages. Depending on the dataset's language, AutoML chooses the appropriate BERT model. For German data, it uses the German BERT model. For English, it uses the English BERT model. For all other languages, it uses the multilingual BERT model.

In the following code, the German BERT model is triggered because the dataset language is specified as `deu`, the three-letter language code for German according to [ISO classification](https://iso639-3.sil.org/code/deu):
