---
title: Data featurization with automated machine learning
...
| [SparseNormalizer](https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.Normalizer.html) | Each sample (that is, each row of the data matrix) with at least one nonzero component is rescaled independently of other samples so that its norm (l1 or l2) equals one. |
...
|**Short series handling** |Passed <br><br><br> Fixed |Automated ML detected enough data points for each series in the input data to continue with training. <br> <br> Automated ML detected that some series didn't contain enough data points to train a model. To continue with training, these short series were dropped or padded.|
