...
ms.date: 03/06/2026
ms.topic: how-to
ms.custom: UpdateFrequency5, cliv1, sdkv1, dev-focus
...
1. After DNS record update, you can validate DNS resolution using *nslookup custom-domain-name* command. If DNS record is correctly updated, the custom domain name points to the IP address of scoring endpoint.
...
If the certificate was originally generated by Microsoft (when using the *leaf_domain_label* to create the service), auto-renewal is no longer available because SDK v1 is deprecated. You must manually renew the certificate. Use one of the following examples to update the certificate:

> [!IMPORTANT]
> * If the existing certificate is still valid, use `renew=True` (SDK) or `--ssl-renew` (CLI) to force the configuration to renew it. This operation takes about 5 hours to take effect.
...
## Next steps
Learn how to:
+ [Configure a secure online endpoint with TLS/SSL (v2)](../how-to-secure-kubernetes-online-endpoint.md)
+ [Upgrade deployment endpoints to SDK v2](../migrate-to-v2-deploy-endpoints.md)
