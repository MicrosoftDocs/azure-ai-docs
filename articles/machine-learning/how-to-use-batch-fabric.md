...
- An [Azure Machine Learning workspace](how-to-manage-workspace.md) that has a model deployment to an Azure Data Lake Storage Gen2 batch endpoint. Fabric supports only hierarchical storage accounts like Azure Data Lake Storage Gen2. For more information, see [Deploy models for scoring in batch endpoints](how-to-use-batch-model-deployments.md).
...
This batch-endpoint pattern is one of several ways to connect OneLake and Azure Machine Learning. For an overview of the integration options, see [Integrate OneLake with Azure Machine Learning](/fabric/onelake/onelake-azure-machine-learning).
...
Create or identify a connection to the storage account that contains the batch endpoint data, so both Fabric OneLake and Azure Machine Learning can access the information. Fabric supports only hierarchical storage accounts like Azure Data Lake Storage Gen2.
...
1. On the **Connection settings** screen, select **New connection**, and then enter the URL for your Azure Data Lake Storage Gen2 account.
...
Create an Azure Machine Learning datastore that points to the storage account. Azure Machine Learning batch endpoints can write predictions only to blob storage accounts, so select **Azure Blob Storage** rather than **Azure Data Lake Storage Gen2** as the **Datastore type** for the batch endpoint. All Azure Data Lake storage accounts are also blob storage accounts.
...
1. On the **Create datastore** screen, provide the following information:
   - **Datastore name**: Enter a name for the datastore. This article uses `trusted_blob` in later steps; if you choose a different name, update the input and output paths accordingly.
...
1. Select the **More actions** icon, select **New subfolder**, and create a folder named **heart-unlabeled-0** to store the sample dataset.
...
1. In the **Value** field, enter the path to the data, *azureml://datastores/trusted_blob/paths/heart-unlabeled-0*.
...
1. To indicate the path to the data, enter *Uri* in the **Name** field.
1. In the **Value** field, enter the following expression to build a unique output path:

   ```
   @concat('azureml://datastores/trusted_blob/paths/endpoints/', pipeline().RunId, '/predictions.csv')
   ```

   Azure Machine Learning batch endpoints support only datastore paths as outputs. Outputs must be unique to avoid conflicts, so you use a dynamic expression that includes the run ID to construct the path.
...
After you configure the settings, run the pipeline and verify the results.

## Run the pipeline and verify results

1. In the pipeline designer, select **Run** to start the pipeline, and confirm the run if prompted.
1. Select the **Output** tab to monitor the Azure Machine Learning activity. Wait for the activity status to show **Succeeded**.
1. To confirm the batch job in Azure Machine Learning, open [Azure Machine Learning studio](https://ml.azure.com), select **Endpoints** > **Batch endpoints**, select your endpoint, and review the completed job.
1. To verify the predictions, return to your Fabric lakehouse and open the OneLake shortcut folder that maps to the output path. Confirm that the *predictions.csv* file appears under the `endpoints/<run-id>` path, and inspect it to see the scored results. The file contains the scored input rows with an appended prediction column.

Because the shortcut and the datastore point to the same storage account, the predictions that the batch endpoint writes are immediately available in Fabric without copying data.

## Troubleshooting

| Symptom | Cause | Resolution |
|:--|:--|:--|
| The batch job fails with a storage mount or access error. | The compute that runs the deployment lacks permission to mount the storage account. | Grant the deployment compute identity access to the storage account. For more information, see [Access storage services](how-to-identity-based-service-authentication.md#access-storage-services). |
| The datastore can't be created, or the endpoint can't write outputs. | The datastore was created as **Azure Data Lake Gen2** instead of **Azure Blob Storage**. | Recreate the datastore and select **Azure Blob Storage** as the **Datastore type**. Batch endpoints write predictions only to blob storage accounts. |
| The pipeline fails to invoke the endpoint with an authorization error. | The connection identity can't call the batch endpoint. | Make sure the organizational account or service principal that the connection uses has permission to invoke the batch endpoint. For more information, see [Authorization on batch endpoints](how-to-authenticate-batch-endpoint.md). |
| The output path is invalid, or a run overwrites previous results. | The dynamic output expression is malformed or not unique. | Verify that the output **Uri** uses a dynamic expression that includes `pipeline().RunId` so each run writes to a unique path. |
