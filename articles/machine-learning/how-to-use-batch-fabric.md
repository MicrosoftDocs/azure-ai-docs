...
ms.custom:
  ...
  - ignite-2023
  - sfi-image-nochange
#Customer intent: As a data science professional and Fabric user, I want to be able to configure Fabric to consume Azure Machine Learning batch deployments, so I can access batch endpoints from both Azure Machine Learning and Fabric.
---
...
In this article, you learn how to use Microsoft Fabric to consume Azure Machine Learning batch deployments. This workflow uses models that are deployed to batch endpoints and also supports batch pipeline deployments from Fabric.
...
1. In the new folder, select **Get data** and then select **Upload files** to upload the sample dataset *heart-unlabeled-0.csv*.
...
Create a Fabric-to-batch inferencing pipeline in your existing Fabric workspace that invokes the batch endpoints.
...
1. Next to **Azure Machine Learning connection**, select the dropdown arrow and then select **Browse all**.
...
1. In the **Connection credentials** section, select **New connection** under **Connection**, and provide a connection name under **Connection name**.
...
1. For **Batch deployment**, select a specific deployment if needed.
   ...
   The **Batch deployment** section automatically populates with the available deployments under the endpoint. If you don't select a deployment, Fabric invokes the **Default** deployment under the endpoint, allowing the batch endpoint creator to decide which deployment to call. In most scenarios, keep this default behavior.

### Configure inputs and outputs for the batch endpoint

Configure inputs and outputs from the batch endpoint. *Inputs* to batch endpoints supply data and parameters needed to run the process. *Outputs* provide the paths to place the batch data.
