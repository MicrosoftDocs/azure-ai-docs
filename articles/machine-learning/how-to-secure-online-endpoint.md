...
1. Create your workspace. The `-m allow_only_approved_outbound` parameter configures a managed virtual network for the workspace and blocks outbound traffic except to approved destinations.
   ...
   > [!IMPORTANT]
   > When a managed virtual network is set up for a workspace for the first time, the network isn't provisioned. For example, when you create a workspace and use the `-m allow_only_approved_outbound` parameter in this step, the network that's configured isn't provisioned. Before proceeding to create online deployments, provision the network by following the guidelines in [Manually provision a managed network](how-to-managed-network.md#manually-provision-a-managed-vnet). You can't create online deployments until the managed network is provisioned.
