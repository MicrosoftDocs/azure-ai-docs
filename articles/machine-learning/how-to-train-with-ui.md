...
You can create a training job in Azure Machine Learning in many ways. You can train models by using the [Azure CLI](how-to-train-model.md), the [REST API](how-to-train-with-rest.md), or you can use the studio UI to directly create a training job.

In this article, you learn how to use your own data and code to train a machine learning model by using a guided experience for submitting training jobs in the Azure Machine Learning studio.
...
1. Select your method of training, and then choose **Start configuring job** to open the submission form.
    ...
    The next section walks through the form with the steps for running a custom training script (command job).

## Configure basic settings

Configure the basic information about your training job. You can proceed to the next page if you're satisfied with the defaults, or make changes based on your desired preference.
...
| Field | Description |
|------| ------|
|Job name| Use the job name to uniquely identify your job. It's also used as the display name for your job.|
|Experiment name| Organize the job in Azure Machine Learning studio. Each job's run record is organized under the corresponding experiment in the studio's **Experiment** tab. By default, Azure puts the job in the **Default** experiment.|
...
|Inputs| Specify as many inputs as needed of the following data types: integer, number, boolean, string. |
...
Run the command from the root directory of the uploaded code folder. After you select your code file or folder, you can see the files to be uploaded. Copy the relative path to the code containing your entry point and paste it into the **Enter the command to start the job** box.

If the code is in the root directory, you can directly refer to it in the command. For instance, `python main.py`.

If the code isn't in the root directory, use the relative path. For example, the structure of the [word language model](https://github.com/Azure/azureml-examples/tree/main/cli/jobs/single-step/pytorch/word-language-model) is:
...
Here, the source code is in the `src` subdirectory. The command is `python ./src/main.py` (plus other command-line arguments).
...
When you use an input in the command, specify the input name. To indicate an input variable, use the form `${{inputs.input_name}}`. For example, `${{inputs.wiki}}`. You can then refer to it in the command, such as `--data ${{inputs.wiki}}`.
...
| --- | --- |
| Serverless | [Model training on serverless compute](how-to-use-serverless-compute.md) |
...
1. When you're satisfied with your choices, select **Next**.
...
| Compute type | How to |
| --- | --- |
| Serverless | [Use serverless compute for training](how-to-use-serverless-compute.md) |
...
After selecting a compute target, specify the runtime environment for your job. The job creation UI supports three types of environments:
...
Curated environments are Azure-defined collections of Python packages used in common ML workloads. You have access to curated environments in your workspace by default. These environments use cached Docker images, which reduce the job preparation overhead. The cards displayed in the **Curated environments** page show details of each environment. To learn more, see [Azure Machine Learning Curated Environments](resource-curated-environments.md).
...
If you don't want to use the Azure Machine Learning curated environments or specify your own custom environment, use a Docker image from a public container registry such as [Docker Hub](https://hub.docker.com).

## Review and create

After you configure the job, select **Next** to go to the **Review** page. To modify a setting, select the pencil icon and make the change.
...
To launch the job, select **Submit training job**. After the job is created, Azure shows you the job details page, where you can monitor and manage your training job.
