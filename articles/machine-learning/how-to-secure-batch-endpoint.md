...
You can secure batch endpoints communication by using private networks. This article explains the requirements to use batch endpoints in an environment secured by private networks.

> [!NOTE]
> Azure Machine Learning also supports [workspace managed virtual network isolation](how-to-managed-network.md), which automates network configuration and is the recommended approach for outbound isolation. The steps in this article describe securing batch endpoints with a custom Azure Virtual Network.

## Securing batch endpoints

Batch endpoints inherit the networking configuration from the workspace where they're deployed. All the batch endpoints created inside of private link-enabled workspace are deployed as private batch endpoints by default. When the workspace is correctly configured, no further configuration is required.
...
1. You have configured your Azure Machine Learning workspace for private networking. For more details about how to achieve it read [Create a secure workspace with a virtual network](tutorial-create-secure-workspace-vnet.md).

    > [!NOTE]
    > For the recommended managed virtual network approach, see [Tutorial: Create a secure workspace with a managed virtual network](tutorial-create-secure-workspace.md).
...
2. For Azure Container Registry in private networks, there are [some prerequisites about their configuration](how-to-secure-workspace-vnet.md#prerequisites).
    ...
    > [!WARNING]
    > Azure Container Registries with the Quarantine feature enabled aren't currently supported.

3. Ensure blob, file, queue, and table private endpoints are configured for the storage accounts as explained at [Secure Azure storage accounts](how-to-secure-workspace-vnet.md#secure-azure-storage-accounts). Batch deployments require all the four to properly work.
...
> [!CAUTION]
> Unlike online endpoints, batch endpoints don't support the `public_network_access` or `egress_public_network_access` keys when configuring the endpoint. It isn't possible to deploy public batch endpoints on private link-enabled workspaces.
...
1. Extra network security groups may be required depending on your case. For more information, see [How to secure your training environment](how-to-secure-training-vnet.md).
...
- When working on a private link-enabled workspace, batch endpoints can be created and managed using Azure Machine Learning studio. However, they can't be invoked from the UI in studio. Use the Azure Machine Learning CLI v2, Python SDK v2, or REST API instead for job creation. For more information, see [Run batch endpoints and access results](how-to-use-batch-model-deployments.md#run-batch-endpoints-and-access-results).

- Batch deployments aren't supported in workspaces with customer-managed encryption keys (CMK).
