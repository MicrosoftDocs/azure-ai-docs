...
To upgrade, change your code for submitting jobs to SDK v2. You don't need to upgrade what you run _within_ the job to SDK v2. However, remove any code specific to Azure Machine Learning from your model training scripts. This separation allows for an easier transition between local and cloud and is considered best practice for mature MLOps. In practice, this means removing `azureml.*` lines of code. Replace model logging and tracking code with MLflow. For more details, see [how to use MLflow in v2](how-to-use-mlflow-cli-runs.md).
