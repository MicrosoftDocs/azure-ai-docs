...
Explore your experimentation results with a dashboard.  The dashboard contains a combination of different tiles – chart visualizations, comparison table, markdown, and more for a view that's dynamic, flexible, and customizable for you to explore your experimentation results.  

The dashboard helps you save time, keep your results organized, and make informed decisions such as whether to retrain or deploy your model.

This article shows you how to use and customize your dashboard with the following tasks:
...
> [!TIP]
> * The Azure Machine Learning CLI v1 reached end of support on September 30, 2025, and SDK v1 is deprecated (end of support June 30, 2026). For v1 reference, see [Track, monitor, and analyze runs (v1)](./v1/how-to-track-monitor-analyze-runs.md).
> * If you're looking for information on monitoring training jobs from the CLI or SDK v2, see [Track experiments and models by using MLflow](how-to-use-mlflow-cli-runs.md).
...
You're now on the default dashboard view where you find your job list consolidated into the left side bar and dashboard content on the right.

If you select a specific experiment, then you'll automatically land into the **Dashboard view**.  

### Jobs list view

The left side bar is a collapsed view of your jobs list. You can filter, add columns, and pin columns by selecting the respective icon next to the search bar.
...
For sweep and AutoML jobs, you can easily identify the best trial and best model with the **Best** label positioned next to the appropriate job display name. This simplifies comparisons across these jobs.

### Sections

The dashboard is made up of sections that can be used to organize different tiles and information.

By default, you find all of your logged training metrics in **Custom metrics** section and resource usage in **Resource metrics** section.

You can perform the following actions:

* Update the section name by selecting the pencil icon when hovering on the section name.
* Move sections up and down and remove sections that you no longer need.
...
By default, the **Custom metrics** and **Resource metrics** sections generate chart tiles for each of the metrics. 
...
To reduce the list to show only jobs that are visualized in the dashboard, select the eye at the top to **Show only visualize**.

To reset and start choosing a new set of jobs to visualize, select the eye at the top to **Visualize None** to remove all jobs from surfacing in the dashboard. Then go ahead and select the new set of jobs.
...
Change the baseline by hovering over the display name and selecting the "baseline" icon. Show differences only reduce the rows in the table to only surface rows that have different values so you can easily spot what factors contributed to the results.
...
>Jobs with a runtime of less than 5 minutes won't have enough data to populate this view.
...
> [!NOTE]
> You can't save changes to the **Default view**, but you can save them into your own **Custom view**. Manage your views from **View options** to create new, edit existing, rename, or delete them.
> 
...
* To learn how to log metrics for your experiments, see [Log metrics, parameters, and files with MLflow](how-to-log-view-metrics.md).
