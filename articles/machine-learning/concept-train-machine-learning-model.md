...
The Azure Machine Learning SDK for Python allows you to build and run machine learning workflows with Azure Machine Learning. You can interact with the service from an interactive Python session, Jupyter Notebooks, Visual Studio Code, or another IDE.
...
A generic training job with Azure Machine Learning can be defined using the [command()](/python/api/azure-ai-ml/azure.ai.ml#azure-ai-ml-command) function. The Command object is then used, along with your training scripts to train a model on the specified compute target.

You're able to start with a Command object for your local computer, and then switch to one for a cloud-based compute target as needed. When changing the compute target, you only change the compute parameter in the Command object that you use. A run also logs information about the training job, such as the inputs, outputs, and logs.
...
Designer lets you train models using a drag and drop interface in your web browser.
