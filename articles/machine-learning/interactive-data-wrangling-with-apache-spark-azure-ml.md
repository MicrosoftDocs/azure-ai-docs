...
The article explains how to attach and configure a serverless Spark compute, and then shows how to use the serverless Spark compute to access and wrangle data from several sources.
...
- A copy of the *titanic.csv* dataset, available at [https://github.com/datasciencedojo/datasets/commits/master/titanic.csv](https://github.com/datasciencedojo/datasets/commits/master/titanic.csv), uploaded to the default file share in your workspace.
...
- Familiarity with the process of storing secrets in Azure Key Vault by using:
  - Azure Blob storage account access key.
  - Shared Access Signature (SAS) tokens.
  - ADLS service principal information.
...
>[!NOTE]
>The code and files for this article, including the *titanic.csv* data file, are available in a Machine Learning notebook at [https://github.com/Azure/azureml-examples/tree/main/sdk/python/data-wrangling](https://github.com/Azure/azureml-examples/tree/main/sdk/python/data-wrangling). To use the notebook, open it in your Azure Machine Learning studio workspace under **Notebooks** > **Samples** and clone it to your workspace **Files** share.
...
You can use either user identity passthrough or service principal-based data access to access and wrangle data stored in Azure Data Lake Storage storage accounts. You use `abfss://` protocol URIs to access the data. User identity passthrough requires the fewest configuration steps.

To use either method, verify that the user identity or service principal has **Contributor** and **Storage Blob Data Contributor** [role assignments](apache-spark-environment-configuration.md#add-role-assignments-in-azure-storage-accounts) in the Azure Data Lake Storage account.

<a name-"user-identity"></a>
### Access and wrangle data with user identity passthrough

The following Titanic data wrangling code sample uses a data URI in format `abfss://<FILE_SYSTEM_NAME>@<STORAGE_ACCOUNT_NAME>.dfs.core.windows.net/<PATH_TO_DATA>` with `pyspark.pandas` and `pyspark.ml.feature.Imputer`. Replace the placeholders with your values.
...
1. In the following code sample, set the service principal tenant ID, client ID, and client secret values.

   The `get_secret()` call in the code depends on name of the key vault and the names of the key vault secrets created for the service principal tenant ID, client ID and client secret. Set these corresponding property names/values in the configuration:
...
1. Run the following code sample.
   ...
   # Set up service principal that has access to the data
...
1. Import and  wrangle the *titanic.csv* data using the data URI in the `abfss://<FILE_SYSTEM_NAME>@<STORAGE_ACCOUNT_NAME>.dfs.core.windows.net/<PATH_TO_DATA>` format, as shown in the code sample for the [user identity passthrough method](#user-identity).
...
- To configure the storage account access key for data access, set the `fs.azure.account.key.<STORAGE_ACCOUNT_NAME>.blob.core.windows.net` property as shown in the following code snippet:
...
1. Run the following data wrangling code. Format the data URI as `wasbs://<BLOB_CONTAINER_NAME>@<STORAGE_ACCOUNT_NAME>.blob.core.windows.net/<PATH_TO_DATA>`.
...
To access data from [Azure Machine Learning Datastore](how-to-datastore.md), define a path to data on the datastore with the [URI format](how-to-create-data-assets.md?tabs=cli#create-data-assets) `azureml://datastores/<DATASTORE_NAME>/paths/<PATH_TO_DATA>`.
...
The Azure Machine Learning datastores can access data using Azure storage account access key, SAS token, or service principal credentials, or they can use credentialless data access. Depending on the datastore type and the underlying Azure storage account type, select an appropriate authentication mechanism to ensure data access. This table summarizes the authentication mechanisms to access data in Azure Machine Learning datastores:
