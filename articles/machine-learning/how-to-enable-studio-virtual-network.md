...
* Azure Storage Account (blob and file)
...
* A preexisting virtual network and subnet to use.
...
* When the storage account is in the virtual network, extra validation requirements exist to use studio:
...
There's a known issue where you can't run a sample pipeline in the designer homepage. This problem occurs because the sample dataset used in the sample pipeline is an Azure Global dataset. You can't access it from a virtual network environment.
...
> [!TIP]
> The first step isn't required for the default storage account for the workspace. All other steps are required for *any* storage account behind the virtual network and used by the workspace, including the default storage account.
...
1. **If the storage account is the *default* storage for your workspace, skip this step**. If it isn't the default, **grant the workspace managed identity the Storage Blob Data Reader role** for the Azure storage account so that it can read data from blob storage.
    ...
    For more information, see the [Blob Data Reader](/azure/role-based-access-control/built-in-roles#storage-blob-data-reader) built-in role.
...
1. **Grant the workspace managed identity the Reader role for storage private endpoints**. If your storage service uses a private endpoint, grant the workspace's managed identity *Reader* access to the private endpoint. The workspace's managed identity in Microsoft Entra ID has the same name as your Azure Machine Learning workspace. A private endpoint is necessary for both blob and file storage types.
    ...
    For more information, see the [Reader](/azure/role-based-access-control/built-in-roles#reader) built-in role.
...
When you use Azure Data Lake Storage Gen2 as a datastore, you can use both Azure RBAC and POSIX-style access control lists (ACLs) to control data access inside of a virtual network.
...
**To use ACLs**, assign the workspace's managed identity access just like any other security principal. For more information, see [Access control lists on files and directories](/azure/storage/blobs/data-lake-storage-access-control#access-control-lists-on-files-and-directories).

## Datastore: Azure SQL Database

To access data stored in an Azure SQL Database with a managed identity, you must create a SQL contained user that maps to the managed identity. For more information on creating a user from an external provider, see [Create contained database users](/azure/azure-sql/database/authentication-aad-configure#contained-database-users).
...
When you use the Azure Machine Learning designer intermediate component output, you can specify the output location for any component in the designer. Use this output to store intermediate datasets in a separate location for security, logging, or auditing purposes. To specify output, use the following steps:

1. Select the component whose output you want to specify.
...
## Access the studio from a resource inside the virtual network

If you access the studio from a resource inside a virtual network (for example, a compute instance or virtual machine), you must allow outbound traffic from the virtual network to the studio.
...
> [!TIP]
> To reduce data exfiltration risk, consider using specific FQDNs instead of the `AzureFrontDoor.Frontend` service tag. Allow outbound access only to `ml.azure.com` and `automlresources-prod-d0eaehh7g8andvav.b02.azurefd.net`. For more information, see [Azure Machine Learning data exfiltration prevention](how-to-prevent-data-loss-exfiltration.md).

## Firewall settings

Some storage services, such as Azure Storage Account, have firewall settings that apply to the public endpoint for that specific service instance. Usually, this setting allows you to allow or disallow access from specific IP addresses from the public internet. **Azure Machine Learning studio doesn't support this setting**. It's supported when using the Azure Machine Learning SDK or CLI.

> [!TIP]
> Azure Machine Learning studio supports using the Azure Firewall service. For more information, see [Configure inbound and outbound network traffic](how-to-access-azureml-behind-firewall.md).
