...
Deploy this model. Deployment takes a few minutes to complete. The deployment process registers the model, autogenerates the scoring script and environment, and provisions the online endpoint.

1. Select **the best model** to open the model-specific page.

1. Select **Deploy** > **Real-time endpoint**.

   Azure Machine Learning generates the model and environment needed for the deployment.

1. In the deployment wizard, provide the following values. Keep the defaults for any fields that aren't listed.

    | Field | Value |
    |----|-----|
    | Endpoint name | Enter a unique name, such as `bikeshare-endpoint`. |
    | Deployment name | Enter a name, such as `bikeshare-deploy`. |
    | Virtual machine | Keep the recommended default virtual machine size. |
    | Instance count | `1` |
    | Authentication type | Keep the default (**Key-based authentication**). |

1. Complete the remaining steps in the wizard, and then select **Deploy**.  

    A notification confirms that the deployment started successfully. Track the progress in the **Model summary** pane under **Deploy status**.
...
1. Select the endpoint you want to delete and select **Delete**. 
