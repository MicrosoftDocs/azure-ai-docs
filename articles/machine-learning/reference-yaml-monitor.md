...
|`start_time`| string |Describes the start date and time with timezone. If `start_time` is omitted, the first job runs instantly and future jobs are triggered based on the schedule, meaning `start_time` equals the job created time. If the start time is in the past, the first job runs at the next calculated run time.|
...
|`timezone`| string |Specifies the time zone of the recurrence. If omitted, defaults to UTC. |See [appendix for timezone values](#timezone)|
|`pattern`|object|Specifies the pattern of the recurrence. If pattern is omitted, jobs are triggered according to the logic of start_time, frequency, and interval.| |
...
| `alert_notification` | Object | Description of alert notification recipients. | The allowed alert destination is an object  `emails`  containing an array of email recipients |  |
...
| `production_data.data_context` | String | The context of data. Refers to model production data, which can be model inputs or model outputs. | `model_inputs` |  |
| `production_data.data_window` | Object | **Optional**. Data window of the production data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `production_data.data_window.lookback_window_offset` and `production_data.data_window.lookback_window_size` properties. For a fixed data window, specify `production_data.data_window.window_start` and `production_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `production_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `production_data.data.input_data.type` is `uri_folder`, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
...
| `reference_data.data_context` | String | The context in which the dataset was previously used. | `model_inputs`, `training`, `test`, `validation` |  |
...
| `reference_data.data_window` | Object | **Optional**. Data window of the reference data to be used as comparison baseline data. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `reference_data.data_window.lookback_window_offset` and `reference_data.data_window.lookback_window_size` properties. For a fixed data window, specify `reference_data.data_window.window_start` and `reference_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `reference_data_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is **required** if `reference_data.input_data.type` is `uri_folder`. For more information, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `features` | Object | **Optional**. Target features to monitor for data drift. Some models might have hundreds or thousands of features. We recommend specifying the features of interest for monitoring. | One of the following values: list of feature names, `features.top_n_feature_importance`, or `all_features` | Default `features.top_n_feature_importance = 10` if `production_data.data_context` is `training`, otherwise default is `all_features` |
| `alert_enabled` | Boolean | Turn on/off alert notification for the monitoring signal. `true` or `false` | | |
| `metric_thresholds` | Object | List of metrics and thresholds properties for the monitoring signal. When the threshold is exceeded and `alert_enabled` is `true`, you receive an alert notification. | | |
| `metric_thresholds.numerical` | Object | **Optional**. List of metrics and thresholds in `key:value` format, where `key` is the metric name and `value` is the threshold. | Allowed numerical metric names: `jensen_shannon_distance`, `normalized_wasserstein_distance`, `population_stability_index`, `two_sample_kolmogorov_smirnov_test`| |
| `metric_thresholds.categorical` | Object | **Optional**. List of metrics and thresholds in `key:value` format, where `key` is the metric name and `value` is the threshold. | Allowed categorical metric names: `jensen_shannon_distance`, `chi_squared_test`, `population_stability_index`| |
...
| `production_data` | Object | **Optional**. Description of production data to be analyzed for monitoring signal. | | |
| `production_data.input_data` | Object | **Optional**. Description of input data source, see [job input data](./reference-yaml-job-command.md#job-inputs) specification. | | |
| `production_data.data_context` | String | The context of data. Refers to model production data, which can be model inputs or model outputs. | `model_outputs` |  |
| `production_data.data_window` | Object | **Optional**. Data window of the production data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `production_data.data_window.lookback_window_offset` and `production_data.data_window.lookback_window_size` properties. For a fixed data window, specify `production_data.data_window.window_start` and `production_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `production_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `production_data.data.input_data.type` is `uri_folder`. For more information on preprocessing component specification, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `reference_data` | Object | **Optional**. Recent past production data is used as comparison baseline data if this isn't specified. Recommendation is to use training data as comparison baseline. | | |
| `reference_data.input_data` | Object | Description of input data source, see [job input data](./reference-yaml-job-command.md#job-inputs) specification. | | |
| `reference_data.data_context` | String | The context in which the dataset was previously used. | `model_inputs`, `training`, `test`, `validation` |  |
| `reference_data.data_column_names.target_column` | Object | **Optional**. If the `reference_data` is training data, this property is required for monitoring top N features for data drift. | | |
| `reference_data.data_window` | Object | **Optional**. Data window of the reference data to be used as comparison baseline data. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `reference_data.data_window.lookback_window_offset` and `reference_data.data_window.lookback_window_size` properties. For a fixed data window, specify `reference_data.data_window.window_start` and `reference_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `reference_data_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is **required** if `reference_data.input_data.type` is `uri_folder`, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `features` | Object | **Optional**. Target features to monitor for prediction drift. Some models might have hundreds or thousands of features. We recommend specifying the features of interest for monitoring. | One of the following values: list of feature names, `features.top_n_feature_importance`, or `all_features` | Default `features.top_n_feature_importance = 10` if `production_data.data_context` is `training`, otherwise default is `all_features` |
| `alert_enabled` | Boolean | Turn on/off alert notification for the monitoring signal. `true` or `false` | | |
| `metric_thresholds` | Object | List of metrics and thresholds properties for the monitoring signal. When the threshold is exceeded and `alert_enabled` is `true`, you receive an alert notification. | | |
| `metric_thresholds.numerical` | Object | **Optional**. List of metrics and thresholds in `key:value` format, where `key` is the metric name and `value` is the threshold. | Allowed numerical metric names: `jensen_shannon_distance`, `normalized_wasserstein_distance`, `population_stability_index`, `two_sample_kolmogorov_smirnov_test`| |
| `metric_thresholds.categorical` | Object | **Optional**. List of metrics and thresholds in `key:value` format, where `key` is the metric name and `value` is the threshold. | Allowed categorical metric names: `jensen_shannon_distance`, `chi_squared_test`, `population_stability_index`| |
...
| `production_data` | Object | **Optional**. Description of production data to be analyzed for monitoring signal. | | |
| `production_data.input_data` | Object | **Optional**. Description of input data source,  see [job input data](./reference-yaml-job-command.md#job-inputs) specification.| | |
| `production_data.data_context` | String | The context of data. Refers to model production data, which can be model inputs or model outputs. | `model_inputs`, `model_outputs` | |
| `production_data.data_window` | Object | **Optional**. Data window of the production data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `production_data.data_window.lookback_window_offset` and `production_data.data_window.lookback_window_size` properties. For a fixed data window, specify `production_data.data_window.window_start` and `production_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
...
| `reference_data` | Object | **Optional**. Recent past production data is used as comparison baseline data if this isn't specified. Recommendation is to use training data as comparison baseline. | | |
| `reference_data.input_data` | Object | Description of input data source, see [job input data](./reference-yaml-job-command.md#job-inputs) specification. | | |
| `reference_data.data_context` | String | The context in which the dataset was previously used. | `model_inputs`, `model_outputs`, `training`, `test`, `validation` |  |
| `reference_data.data_column_names.target_column` | Object | **Optional**. If the `reference_data` is training data, this property is required for monitoring top N features for data quality. | | |
| `reference_data.data_window` | Object | **Optional**. Data window of the reference data to be used as comparison baseline data. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `reference_data.data_window.lookback_window_offset` and `reference_data.data_window.lookback_window_size` properties. For a fixed data window, specify `reference_data.data_window.window_start` and `reference_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `reference_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `reference_data.input_data.type` is `uri_folder`, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `features` | Object | **Optional**. Target features to monitor for data quality. Some models might have hundreds or thousands of features. We recommend specifying the features of interest for monitoring. | One of the following values: list of feature names, `features.top_n_feature_importance`, or `all_features` | Default `features.top_n_feature_importance = 10` if `reference_data.data_context` is `training`, otherwise default is `all_features` |
| `alert_enabled` | Boolean | Turn on/off alert notification for the monitoring signal. `true` or `false` | | |
| `metric_thresholds` | Object | List of metrics and thresholds properties for the monitoring signal. When the threshold is exceeded and `alert_enabled` is `true`, you receive an alert notification. | | |
...
| `production_data` | Array | **Optional**. Defaults to collected data associated with Azure Machine Learning endpoint if not provided. The `production_data` is a list of datasets and associated metadata. It must include both model inputs and model outputs data. It can be a single dataset with both model inputs and outputs, or two separate datasets containing model inputs and model outputs.| | |
| `production_data.input_data` | Object | **Optional**. Description of input data source. For more information, see [job input data](./reference-yaml-job-command.md#job-inputs) specification.| | |
...
| `production_data.data_context` | String | The context of data. Refers to production model inputs data. | `model_inputs`, `model_outputs`, `model_inputs_outputs` |  |
| `production_data.data_window` | Object | **Optional**. Data window of the production data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `production_data.data_window.lookback_window_offset` and `production_data.data_window.lookback_window_size` properties. For a fixed data window, specify `production_data.data_window.window_start` and `production_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `production_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `production_data.input_data.type` is `uri_folder`. For more information, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `production_data.data_window_size` | String |**Optional**. Data window size in days with ISO8601 format, for example `P7D`. This is the production data window to be computed for feature attribution drift. | By default, the data window size is the last monitoring period.| |
| `reference_data` | Object | **Optional**. Recent past production data is used as comparison baseline data if this isn't specified. Recommendation is to use training data as comparison baseline. | | |
| `reference_data.input_data` | Object | Description of input data source, see [job input data](./reference-yaml-job-command.md#job-inputs) specification. | | |
| `reference_data.data_context` | String | The context in which the dataset was previously used. For feature attribution drift, only `training` data is allowed. | `training` |  |
| `reference_data.data_column_names.target_column` | String | **Required**. | | |
| `reference_data.data_window` | Object | **Optional**. Data window of the reference data to be used as comparison baseline data. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `reference_data.data_window.lookback_window_offset` and `reference_data.data_window.lookback_window_size` properties. For a fixed data window, specify `reference_data.data_window.window_start` and `reference_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `reference_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `reference_data.input_data.type` is `uri_folder`. For more information, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `alert_enabled` | Boolean | Turn on/off alert notification for the monitoring signal. `true` or `false` | | |
| `metric_thresholds` | Object | Metric name and threshold for feature attribution drift in `key:value` format, where `key` is the metric name and `value` is the threshold. When the threshold is exceeded and `alert_enabled` is `true`, you receive an alert notification. | Allowed metric name: `normalized_discounted_cumulative_gain` | |
...
| `input_data.<data_name>.data_context` | String | The context of data. Refers to model production data, which can be model inputs or model outputs. | `model_inputs` |  |
| `input_data.<data_name>.data_window` | Object | **Optional**. Data window of the input data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `input_data.<data_name>.data_window.lookback_window_offset` and `input_data.<data_name>.data_window.lookback_window_size` properties. For a fixed data window, specify `input_data.<data_name>.data_window.window_start` and `input_data.<data_name>.data_window.window_end` properties. All property values must be in ISO8601 format. | |
...
| `production_data.data_window` | Object | **Optional**. Data window of the production data to be analyzed. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `production_data.data_window.lookback_window_offset` and `production_data.data_window.lookback_window_size` properties. For a fixed data window, specify `production_data.data_window.window_start` and `production_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `production_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `production_data.input_data.type` is `uri_folder`. For more information, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `production_data.data_window_size` | String |**Optional**. Data window size in days with ISO8601 format, for example `P7D`. This is the production data window to be computed for model performance. | By default, the data window size is the last monitoring period.| |
| `reference_data` | Object | **Optional**. Recent past production data is used as comparison baseline data if this isn't specified. Recommendation is to use training data as comparison baseline. | | |
| `reference_data.input_data` | Object | Description of input data source, see [job input data](./reference-yaml-job-command.md#job-inputs) specification. | | |
| `reference_data.data_context` | String | The context in which the dataset was previously used. For model performance, only `training` data is allowed. | `training` |  |
| `reference_data.data_column_names.target_column` | String | **Required**. | | |
| `reference_data.data_window` | Object | **Optional**. Data window of the reference data to be used as comparison baseline data. | Allows either a rolling data window or a fixed data window. For a rolling data window, specify `reference_data.data_window.lookback_window_offset` and `reference_data.data_window.lookback_window_size` properties. For a fixed data window, specify `reference_data.data_window.window_start` and `reference_data.data_window.window_end` properties. All property values must be in ISO8601 format. | |
| `reference_data.pre_processing_component` | String | Component ID in the format of `azureml:myPreprocessing@latest` for a registered component. This is required if `reference_data.input_data.type` is `uri_folder`. For more information, see [preprocessing component specification](how-to-monitor-model-performance.md#set-up-model-monitoring-for-production-data). | | |
| `alert_enabled` | Boolean | Turn on/off alert notification for the monitoring signal. `true` or `false` | | |
