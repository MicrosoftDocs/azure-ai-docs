...
The following properties on a [custom skill](cognitive-search-custom-skill-web-api.md) are used for scale. Review the [custom skill interface](cognitive-search-custom-skill-interface.md) for an introduction to the inputs and outputs that a custom skill should implement.

1. Set `batchSize` on the custom skill to configure the number of records sent in a single skill invocation.

1. Set the `degreeOfParallelism` to calibrate the number of concurrent requests the indexer makes to your skill.

1. Set `timeout` to a value sufficient for the skill to return a valid response.
...
### Considerations
 ...
 There's no one-size-fits-all recommendation. Plan to test different configurations to reach an optimal result. Scale-up strategies are based on either fewer large requests or many small requests.
...
+ For `degreeOfParallelism`, use the average number of requests an indexer batch can generate to guide how you set this value. If the infrastructure hosting the skill (for example, an Azure Function) can't support high levels of concurrency, consider dialing down the degree of parallelism. You can test your configuration with a few documents to validate your understanding of the average number of requests.

+ Although your objective is to scale and support high volumes, testing with a smaller sample of documents helps quantify different stages of execution. For example, you can evaluate your skill's execution time relative to the overall time required to process a subset of documents. This helps you answer the question: does your indexer spend more time building a batch or waiting for a response from your skill?
...
Any item in a batch that errors causes the corresponding document to fail. If you need the document to succeed, return a warning.

Any status code over 299 is evaluated as an error and all enrichments fail, resulting in a failed document.
...
+ The skill returns a valid HTTP status code
...
+ Monitor custom skills with detailed failure logs because data variability can cause specific requests to fail consistently.
