...
> [!NOTE]
> Indexer execution on Standard 3 High Density (S3 HD) search services and serverless indexes follows a different model that includes a service-level daily runtime quota. For details, see [About indexer execution on serverless and S3 HD](search-indexer-high-density-serverless-overview.md).
