...
description: Troubleshoot common questions about Azure AI Search storage metrics that appear inconsistent across the Azure portal, REST APIs, and Azure SDKs.
author: haileytap
...
## Why doesn't storage change immediately after I delete or update documents?
...
The Azure portal and the REST APIs can report different values because they have different refresh cadences. Specifically:
...
A rebuilt index can temporarily show a different storage profile while background merge operations run. Several factors also affect final index size:
...
The index size stabilizes after merge operations complete. However, overall index storage size is nondeterministic, so there could be minor differences between the old and new index storage size.
...
## Why doesn't `vectorIndexSize` match total storage?

`storageSize` and `vectorIndexSize` measure different things:

- `storageSize` is the total disk footprint of an index, including content of all data types. These data types include text, metadata, and vectors.

- `vectorIndexSize` is a limit on the size of a vector index loaded into memory. Vector fields that use the exhaustive KNN algorithm don't consume vector index quota and report zero for `vectorIndexSize`.

On disk, the total storage consumed by vectors can be larger than the in-memory vector index size because Azure AI Search stores multiple copies of vector fields for different purposes. In addition to the vector index itself, the service can store source vectors (JSON data) used for incremental updates and retrievable results, plus original full-precision vectors used for rescoring compressed results. You can choose to eliminate these optional vector copies to reduce disk consumption.

For more information, see [Vector index size and limits](vector-search-index-size.md) and [Eliminate optional vector instances from storage](vector-search-how-to-storage-options.md).
...
## When is a discrepancy expected versus an actual defect?
...
Provide the following information in your [support request](https://portal.azure.com/#view/Microsoft_Azure_Support/HelpAndSupportBlade/~/overview/newsupportrequest/):
