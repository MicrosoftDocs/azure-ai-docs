...
`searchSingle.js` demonstrates a basic scenario where you want to find document descriptions that closely match the search string.
...
For a geo filter, you can specify a geospatial filter to limit results to a specific geographic area. `searchSingleWithFilterGeo.js` limits results to hotels within 300 kilometers of Washington D.C.
...
[Hybrid search](../../hybrid-search-overview.md) combines keyword and vector queries in one request. `searchHybrid.js` runs the full-text and vector query strings concurrently.
...
This example adds [semantic ranking](../../semantic-search-overview.md) to rerank results based on language understanding. `searchSemanticHybrid.js` adds semantic ranking.
