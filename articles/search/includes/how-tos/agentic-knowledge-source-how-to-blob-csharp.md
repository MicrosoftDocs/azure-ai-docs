...
> [!TIP]
> To enforce document-level permissions at query time, set `IngestionPermissionOptions` to `["rbacScope"]` (for Azure RBAC on blob containers) or `["userIds", "groupIds"]` (for ADLS Gen2 ACLs) when you create the knowledge source. At query time, pass the user's access token in the `xMsQuerySourceAuthorization` parameter. For details, see [Enforce permissions at query time](../../agentic-retrieval-how-to-retrieve.md#enforce-permissions-at-query-time).
