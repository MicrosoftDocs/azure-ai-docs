...
| `ai_services` | A Microsoft Foundry resource to access Azure Content Understanding in Foundry Tools. Setting this parameter requires that `contentExtractionMode` is set to `standard`. The endpoint is provided using the `uri` property.  | Object | Only `api_key` is editable | No |
