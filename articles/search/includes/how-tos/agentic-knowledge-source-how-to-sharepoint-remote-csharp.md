...
Remote SharePoint enforces SharePoint permissions at query time. The retrieval engine uses the caller's access token, passed via the `xMsQuerySourceAuthorization` parameter, to query SharePoint content on behalf of the user through the Copilot Retrieval API. Only content the user has access to is returned. SharePoint permissions and Microsoft Purview sensitivity labels are honored in requests for content.

Because remote SharePoint doesn't use a search index, no ingestion-time permissions configuration is needed. The `xMsQuerySourceAuthorization` parameter is the only requirement.
