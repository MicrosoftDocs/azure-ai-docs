...
Remote SharePoint enforces SharePoint permissions at query time. The retrieval engine uses the caller's access token, passed via the `x_ms_query_source_authorization` parameter, to query SharePoint content on behalf of the user through the Copilot Retrieval API. Only content the user has access to is returned. SharePoint permissions and Purview sensitivity labels are honored in requests for content.
