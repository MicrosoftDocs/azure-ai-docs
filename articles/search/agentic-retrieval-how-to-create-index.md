...
Vector fields are used only for query execution on Azure AI Search. You don't need the vector in results because it isn't human or LLM readable. To minimize space requirements, set `retrievable` and `stored` to false. For more information, see [Optimize vector storage and processing](vector-search-how-to-configure-compression-storage.md).
