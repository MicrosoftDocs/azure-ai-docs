---
title: Configure CMK across different tenants in Azure AI Search by using federated identity credentials (preview)
description: Configure cross-tenant customer-managed key encryption in Azure AI Search using a managed identity with federated identity credentials.
author: mattwojo
ms.author: mattwoj
ms.reviewer: mcarter
ms.date: 03/20/2026
...
# Configure customer-managed keys across different tenants by using federated identity credentials (preview)

[!INCLUDE [Feature preview](./includes/previews/preview-generic.md)]

This article explains how to [configure customer-managed keys for data encryption in Azure AI Search](search-security-manage-encryption-keys.md) when your search service and Azure Key Vault are in different Microsoft Entra tenants. The recommended approach uses a multitenant Microsoft Entra application with a federated identity credential (FIC) paired with a user-assigned managed identity, so Azure AI Search can access encryption keys in another tenant without client secrets.
...
- A multitenant app registration with a federated identity credential that trusts your search service's managed identity.
- A service principal in the key vault tenant with permission to perform wrap and unwrap operations.
- A search index encrypted with a customer-managed key from a cross-tenant key vault.

A legacy approach that uses a client secret is still supported but no longer recommended. For more information, see [Configure customer-managed keys across different tenants (legacy approach)](./search-security-managed-encryption-cross-tenant.md).

> [!IMPORTANT]
> This article describes the recommended approach by using federated identity credentials with API version `2026-03-01-preview` or later. The previous secret-based approach that used `accessCredentials` with an application ID and client secret is no longer recommended for new configurations and will be deprecated in a future release.
...
- [Permissions in both tenants](#required-permissions) to create applications, assign roles, and manage key vaults.
...
> [!IMPORTANT]
> The `federatedIdentityClientId` property requires API version `2025-11-01-preview` or later.
...
```http
GET https://<search-service>.search.windows.net/indexes/cmk-demo?api-version=2025-11-01-preview
api-key: <ADMIN-API-KEY>
```
