...
When you chunk data based on fixed size, overlapping a small amount of text between chunks can help maintain continuity and context. We recommend starting with a chunk size of 512 tokens (approximately 2,000 characters) and an initial overlap of 25%, which equals 128 tokens. This overlap ensures smoother transitions between chunks without excessive duplication.

The optimal overlap might vary depending on your content type and use case. For example, highly structured data might require less overlap, while conversational or narrative text might benefit from more.
