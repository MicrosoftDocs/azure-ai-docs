...
In Search explorer, POST requests are formulated internally using [Documents - Search Post](/rest/api/searchservice/documents/search-post) (REST API), with responses returned as verbose JSON documents.
...
To learn more about query structures and syntax, use a REST client to create query expressions that use more parts of the REST API. [Documents - Search Post](/rest/api/searchservice/documents/search-post) (REST API) is especially helpful for learning and exploration.
