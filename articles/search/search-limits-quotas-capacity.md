...
Quota and capacity failures can come from five separate controls: subscription quota, regional deployability, service or index limits, storage or vector headroom, and quota or throttling on a dependent service. Use evidence from the completed operation to choose one path.

Before you use the table, check whether the search service is in a `Provisioning` state. If a create, scale, or upgrade operation is still running, wait for the provisioning state to become `Succeeded` or `Failed`. An operation in progress isn't evidence of a quota, regional deployability, or service limit problem. If the completed operation fails, use the following table to classify the result.

| Failure | Evidence to check | Classification | Action | Escalation route |
| --- | --- | --- | --- | --- |
| You can't create another search service in a subscription and region. | In Azure **Quotas**, compare current usage and the limit for the requested tier and region. Record the requested service count and the difference between usage and the limit. | **Subscription quota** applies when current usage plus the requested count exceeds the limit. | [Request a quota adjustment](search-create-service-portal.md#add-more-services-to-your-subscription). You can also reduce the service count or use another subscription or region. See [Subscription limits](#subscription-limits). | If the quota workflow rejects or doesn't complete the request, use the support path that the workflow provides or open an Azure support request. Include the subscription, region, tier, current usage, requested limit, UTC time, and available request or correlation ID. |
| You can't create, scale, or upgrade a service even though subscription quota has headroom and the request is within documented limits. | Preserve the full error, operation name, region, tier, requested replicas and partitions, final provisioning state, UTC time, and available operation identifiers. Use [region availability](search-region-support.md) and [tier availability](search-sku-tier.md#region-availability-by-tier) to verify product availability, not live capacity. | **Regional deployability** applies only when an Azure-visible allocation or SKU-availability signal, Azure status guidance, or support confirms that a valid request can't allocate infrastructure. Quota headroom alone doesn't prove regional capacity. If the error doesn't identify a capacity condition, troubleshoot that error without classifying it as regional deployability. | Retry only when an Azure-visible message, Azure status guidance, or support identifies a transient condition. Otherwise, [evaluate an alternative region](search-region-capacity.md#capacity-constraint-options) for your feature, model, network, data residency, availability zone, and compliance requirements. | Open an Azure support request if the operation continues to fail and no suitable region is available. Include the deployment evidence. Don't request more quota only because infrastructure isn't deployable. |
| You can't add replicas or partitions, change tiers, create an index or another object, or a Search endpoint returns a limit or throttling response. | Compare the requested configuration, request rate, and object counts with the tier, service creation date, region, replica and partition count, [service limits](#service-limits), [index limits](#index-limits), and [Search throttling limits](#throttling-limits). | **Service or index limit** applies when the requested configuration, object count, or Search request rate exceeds a documented boundary. | Reduce the request or object count, choose a supported replica and partition combination, add Search capacity, or evaluate another tier. For scale failures, see [Errors during scaling](search-capacity-planning.md#errors-during-scaling). | Open an Azure support request only when observed enforcement disagrees with the documented limit. Include the Search response, service configuration, relevant object counts or request rate, UTC time, and correlation ID. |
| Indexing or ingestion fails near a storage or vector limit. | Capture [GET Service Statistics and GET Index Statistics](vector-search-index-size.md#check-vector-index-size) in the same UTC window. Compare `storageSize` with its quota separately from `vectorIndexSize` and its quota at service and index scope. | **Storage or vector headroom** applies because storage is a disk constraint, while vector index size is a memory constraint for applicable vector fields. Per-index limits can apply independently of service totals. | Add partitions when supported, reduce indexed content or vector size, or use documented vector compression and storage-reduction options. Allow for documented background reclamation after deletes or updates. See [Vector index size limits](#vector-index-size-limits). | Follow the [storage metrics support checklist](troubleshoot-storage-metrics.md#what-should-i-include-in-a-support-request) when aligned measurements don't converge or enforcement disagrees with reported usage. Include raw statistics, write and delete volume, UTC samples, and operational impact. |
| An indexer, skill, vectorizer, or higher-level workflow reports a 429 or quota error from a called service. | Verify that the error originated from Azure OpenAI, Microsoft Foundry, Azure Storage, Azure Machine Learning, or another called resource instead of the Search endpoint. Check indexer execution history and monitoring for the named dependency. | **Dependent-service quota or throttling** applies only after you identify the non-Search component that issued the error. | Follow the named dependency's quota and monitoring guidance. For example, see [Azure OpenAI quotas and limits](/azure/ai-services/openai/quotas-limits) and [integrated vectorization limitations](vector-search-integrated-vectorization.md#limitations). Search retries don't create quota for the dependency. | Route the quota request to the dependency that issued the error. Use Azure AI Search support only for a Search-side limit, Search retry behavior, or a mismatch after you verify the dependency quota. |
