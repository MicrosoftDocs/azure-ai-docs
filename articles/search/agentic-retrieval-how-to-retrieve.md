...
| [Fabric Data Agent](agentic-knowledge-source-how-to-fabric-data-agent.md#enforce-permissions-at-query-time) | ❌ | Your Microsoft Fabric Data Agent is called directly using the user's token. <!-- TO-DO (PM): Confirm whether the Fabric Data Agent enforces per-user data permissions using this token (for example, Power BI row-level security), or whether the token is required solely for authentication. --> |
| [Fabric Ontology](agentic-knowledge-source-how-to-fabric-ontology.md#enforce-permissions-at-query-time) | ❌ | The Fabric ontology MCP endpoint is called using the user's token via an on-behalf-of exchange. |
| [Fabric Data Agent](agentic-knowledge-source-how-to-fabric-data-agent.md#enforce-permissions-at-query-time) | ❌ | Your Microsoft Fabric Data Agent is called directly using the user's token. <!-- TO-DO (PM): Confirm whether the Fabric Data Agent enforces per-user data permissions using this token (for example, Power BI row-level security), or whether the token is required solely for authentication. --> |
| [Fabric Ontology](agentic-knowledge-source-how-to-fabric-ontology.md#enforce-permissions-at-query-time) | ❌ | The Fabric ontology MCP endpoint is called using the user's token via an on-behalf-of exchange. |
