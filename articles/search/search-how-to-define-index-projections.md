...
- Two or more indexes, where the parent index has fields related to the parent document, and the child index is organized around chunks. The child index is the primary search corpus, but the parent index can be used for [lookup queries](/rest/api/searchservice/documents/get) when you want to retrieve the parent fields of a particular chunk. 

Most implementations are a single index inclusive of all parent-child fields, but the system is designed to support separate and multiple child indexes if that's your requirement. Azure AI Search doesn't support index joins so your application code must handle which index to use.
...
- A skillset that includes the Text Split skill. For integrated vectorization, it also has an embedding skill, such as [AzureOpenAIEmbedding](cognitive-search-skill-azure-openai-embedding.md). 

The skillset contains the indexer projection that shapes the data for one-to-many indexing.
...
Your data can originate from any supported data source, but the assumption is that the content is large enough that you want to chunk it, and the reason for chunking it is that you're implementing a RAG pattern that provides grounding data to a chat model.
...
Whether you create one index that combines parent-child fields, or a [two index solution](#example-of-separate-parent-child-indexes) for separating parent-child fields, the primary index used for searching is designed around data chunks. It must have the following fields:
...
Because it's the most common pattern, let's start by reviewing an index schema that combines parent-child fields in a single index.

The following schema is an example that meets the requirements for index projections. Parent fields are the parent_id and the title. Child fields are the vector and nonvector vector chunks. The chunk_id is the document ID of this index. The parent_id and title repeat for every chunk in the index.
...
|-----------|------------|
| `targetIndexName` | The name of the index into which index data is projected. It's either the sole index for a combined parent-child schema, or it's the child index in a [two-index approach](#example-of-separate-parent-child-indexes). |
...
This section shows examples for separate parent and child indexes. Each schema has the fields for its particular grain, with the parent ID field common to both indexes for use in a [lookup query](/rest/api/searchservice/documents/get). The primary search corpus is the child index, but then issue a lookup query to retrieve the parent fields for each match in the result. Your application code or orchestration layer would need to merge or collate results that can be passed to a model.
...
For data sources that provide change tracking and deletion detection, an indexer process can synchronize the primary and secondary indexes to pick up changes in your source data. Each time you run the indexer and skillset, the index projections are updated if the skillset or underlying source data has changed. Any changes picked up by the indexer are propagated through the enrichment process to the projections in the index, ensuring that your projected data is a current representation of content in the originating data source. Data refresh activity is captured in a projected key value for each chunk. This value gets updated when the underlying data changes.
...
If a parent document is changed such that the data within a projected index document changes (for example, if a word is changed in a particular page but no net new pages are added), the data in the target index for that particular projection is updated to reflect that change.

If a parent document is changed such that there are new child documents that weren't there before (for example, if one or more pages worth of text are added to the document), those new child documents are added next time the indexer runs.
