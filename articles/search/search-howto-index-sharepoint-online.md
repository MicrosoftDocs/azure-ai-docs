...
| includeFolder | In the case your URL Pattern has spaces, the library param will not work. You have to substitude the spaces to %20 scape characters to the docs folder. You can copy from your web browser the URL and update it according to it.: <br><br>Example 1 (original URL): <br><br>```"container" : { "name" : "useQuery", "query" : "includeFolder=https://mycompany.sharepoint.com/mysite/MyDocumentFolder%20with%20spaces" }``` <br><br>Example 2 (URI copied from your browser): <br><br>```"container" : { "name" : "useQuery", "query" : "includeFolder=https://mycompany.sharepoint.com/teams/mysite/MyDocumentLibrary/Forms/AllItems%20in%20the%20folder.aspx" }``` |
