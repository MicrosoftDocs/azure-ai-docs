...
> [!NOTE]
> ADLS Gen2 supports [access control model](/azure/storage/blobs/data-lake-storage-access-control) with both Azure role-based access control (Azure RBAC) and POSIX-like access control lists (ACLs) at the blob level. Azure AI Search now supports [Document-Level Permission]() from these ADLS Gen2 access control models, and provides compliant and secure content retrieval experience according to individual user access from the source files. For how to ingest ACLs and RBAC scope using indexer, check [Indexing Access Control Lists and Azure Role-Based Access Control scope using Indexers](search-indexer-access-control-lists-and-role-based-access.md)
