...
In rare cases, such as during maintenance or transient conditions, multiple indexer runs may be queued. When this occurs, the indexer may execute pending workloads sequentially within the scheduled window. For example, if an indexer is scheduled to run hourly and several runs were delayed or triggered on-demand, those runs may execute back-to-back to catch up within that period. These are not additional runs, but they represent previously scheduled or requested executions. While this behavior is uncommon in most scenarios, the indexer is designed to eventually process all queued tasks to maintain consistency and data freshness.
