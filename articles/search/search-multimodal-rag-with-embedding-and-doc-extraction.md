---
title: 'Tutorial: Indexing blobs with text and images for multi-modal RAG scenarios using multi-modal embedding models and document extraction skill'
titleSuffix: Azure AI Search
description: Learn how to extract, index, and search both text and images from Azure Blob Storage for multi-modal RAG scenarios using the Azure AI Search REST APIs.
...
# Tutorial: Indexing blobs with text and images for multi-modal RAG scenarios using multi-modal embedding models and document extraction skill
...
> [!NOTE] 
> Make sure your Search service is running in a [supported region for AI Vision](/azure/ai-services/computer-vision/overview-image-analysis#region-availability)
...
Location: https://<YOUR-SEARCH-SERVICE-NAME>.search.windows-int.net:443/datasources('doc-extraction-multimodal-embedding-ds')?api-version=2025-05-01-Preview
...
+ The Azure AI Vision multimodal embeddings skil enables embedding of both textual and visual data using the same skill type, differentiated by input (text vs image).

+ For more information, see [Azure AI Vision multimodal embeddings skill](/cognitive-search-skill-vision-vectorize.md).
...
```http
### Query the index
POST {{baseUrl}}/indexes/doc-extraction-multimodal-embedding-index/docs/search?api-version=2025-05-01  HTTP/1.1
  Content-Type: application/json
  api-key: {{apiKey}}
...

{
  ...
  "@odata.nextLink": "https://<YOUR-SEARCH-SERVICE-NAME>.search.windows.net/indexes/doc-extraction-multimodal-embedding-index/docs/search?api-version=2025-05-01"
}
```
...
```http
### Reset the indexer
POST {{baseUrl}}/indexers/doc-extraction-multimodal-embedding-indexer/reset?api-version=2025-05-01  HTTP/1.1
  api-key: {{apiKey}}
```

```http
### Run the indexer
POST {{baseUrl}}/indexers/doc-extraction-multimodal-embedding-indexer/run?api-version=2025-05-01  HTTP/1.1
  api-key: {{apiKey}}
```

```http
### Check indexer status 
GET {{baseUrl}}/indexers/doc-extraction-multimodal-embedding-indexer/status?api-version=2025-05-01  HTTP/1.1
  api-key: {{apiKey}}
```
