...
After you deploy the model, the **AML** skill integrates the model into a skillset. You can call the **AML** skill with a stable REST API version or an equivalent Azure SDK. For connections to the model catalog, use a preview API version.

## AML skill usage

Like other built-in skills, a custom **AML** skill has inputs and outputs. The inputs are sent to a deployed AML online endpoint as a JSON object. The output of the endpoint must be a JSON payload in the response, along with a success status code. The response should provide the outputs specified by your **AML** skill definition. Any other response is considered an error, and no enrichments are performed.
...
Currently, the **AML** skill only supports [serverless deployments](/azure/ai-foundry/how-to/deploy-models-serverless.md) of `Cohere-embed-v3-english`, `Cohere-embed-v3-multilingual`, and `Cohere-embed-v4` models from the Azure AI Foundry model catalog.
...
* The model endpoint for a serverless deployment from the catalog, or the [online endpoint (real-time)](../machine-learning/concept-endpoints-online.md) of your AML workspace for a custom model.
