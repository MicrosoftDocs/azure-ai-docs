...
Currently, images extracted using `imageAction: generateNormalizedImages` will always produce a single polygon, but the double-nested structure is maintained for consistency with skills that support multiple polygons, such as the [Azure Content Understanding skill](cognitive-search-skill-content-understanding.md).
