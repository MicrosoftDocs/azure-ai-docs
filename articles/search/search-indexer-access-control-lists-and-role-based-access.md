...
[Azure AI Search ADLS Gen2 indexers](search-howto-index-azure-data-lake-storage.md) enable the ingestion of access control metadata, such as Access Control Lists (ACLs) and Azure Role-Based Access Control (RBAC) scope, directly from Azure Data Lake Storage (ADLS) Gen2. This functionality helps align document-level permissions in the search index with the access controls defined in ADLS Gen2, allowing users to retrieve content in a way that reflects their existing permissions.
