...
| Too many valid agent revisions | 400 | `UserError` | Delete older revisions, then create a new version. This error is terminal—retrying the same create call without deleting revisions keeps failing. |
...
- **Manage agent revisions.** Each agent allows up to 1,000 valid revisions. Delete or rotate revisions you no longer need as part of your deploy pipeline so active agents stay well under the cap, and don't retry create calls that fail with the revision-cap error until you free capacity.
