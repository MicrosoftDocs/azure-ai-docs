...
> on the target Foundry project or target agent. Prefer this role for
> least-privilege access. The **Foundry User** role also works but is intended
> for developers building agents and includes permissions beyond what
> consumers need.
...
| Consent completes but tool calls fail | The user doesn't have permissions in the underlying service | Confirm the user has the required permissions in the underlying service. Also confirm the user has the **Foundry Agent Consumer** role on the [target Foundry project or target agent](../../concepts/rbac-foundry.md#agent-scope-role-assignments). |
