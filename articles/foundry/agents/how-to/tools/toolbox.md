...
For the complete implementation, including the `BearerTokenHandler` that attaches a fresh Foundry bearer token to each request, see the [Skills in Toolbox sample](https://github.com/microsoft-foundry/foundry-samples/tree/main/samples/csharp/hosted-agents/agent-framework/foundry-toolbox-mcp-skills).
