...
Direct traffic between Azure and Microsoft services is expected to use the Microsoft global network, as described in [Global routing and interconnection](/azure/networking/microsoft-global-network#global-routing-and-interconnection). However, customer-configured routes can introduce an on-premises network, firewall, or another intermediary into the path. Hosting the agent in Azure therefore doesn't, by itself, confirm the actual traffic path for a specific deployment. Validate the deployment's effective routes and outbound connectivity configuration.
