...
Direct traffic between Azure and Microsoft services is expected to use the Microsoft global network, as described in [Global routing and interconnection](/azure/networking/microsoft-global-network#global-routing-and-interconnection). However, customer-configured routes can send traffic through an on-premises network, firewall, or another intermediary. Hosting the agent in Azure therefore doesn't, by itself, confirm the actual traffic path for a specific deployment.

Work IQ uses the public HTTPS endpoint `https://workiq.svc.cloud.microsoft`. Authentication and supported file operations can also require outbound HTTPS access to their respective endpoints. A public endpoint can still be reached over the Microsoft global network and doesn't necessarily mean that traffic traverses the public internet. Review and allow the destinations required by your Work IQ scenario.

When virtual network isolation is enabled for the Foundry project, Work IQ tool calls route through the project's single-tenant data proxy. The data proxy is a platform-managed networking component dedicated to the project that handles outbound agent connectivity. Customer-defined network controls can then govern this outbound traffic. For more information about the data proxy and request path, see [Deep dive into Foundry Agent Service networking](../../concepts/agents-networking-deep-dive.md#how-traffic-flows). For the available inbound and outbound configurations, see [Networking options for Foundry Agent Service](../../concepts/networking-options.md#networking-options).

A private endpoint for Foundry provides private inbound access to the Foundry endpoint. It doesn't make the outbound connection from Foundry to Work IQ private. Work IQ remains a public HTTPS destination unless Work IQ provides a supported private-ingress option. The data proxy routing also doesn't change the Microsoft 365 compliance boundary that applies to Work IQ processing.
