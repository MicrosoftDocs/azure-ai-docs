...
The following custom policy definition sets the logging state on every
...
```json
{
  "properties": {
    ...
    "description": "Automatically configures the Agent 365 logging setting on all Microsoft.CognitiveServices/accounts resources.",
    "policyType": "Custom",
    "mode": "Indexed",
    "parameters": {
      "a365LoggingEnabled": {
        ...
        "metadata": {
          ...
          "description": "Set to true to send agent activity data to Agent 365, or false to stop data collection."
        },
        "allowedValues": [
          true,
          false
          ...
          "operations": [
            {
              ...
              "field": "properties.a365LoggingEnabled",
              "value": "[parameters('a365LoggingEnabled')]"
            }
          ]
        }
      }
...
az policy assignment create \
  ...
  --mi-system-assigned \
  --location <location> \
  --params '{ "a365LoggingEnabled": { "value": false } }'
```
...
To disable data collection across all Foundry resources in the subscription,
set the `a365LoggingEnabled` parameter to `false` in the policy assignment.
