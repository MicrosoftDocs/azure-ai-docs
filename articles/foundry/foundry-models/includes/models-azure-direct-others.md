...
MAI-image-2 is a Microsoft-owned, enterprise-ready text-to-image model optimized for photorealistic and production-quality creative outputs. To work with MAI-image-2 in Foundry, see [Deploy and use MAI-image-2 in Microsoft Foundry](../how-to/use-foundry-models-mai.md).

| Model  | Type | Capabilities | Deployment type (region availability) |
| ------ | ---- | ------------ | ------------------------------------- |
| `MAI-image-2` | Text-to-Image <br /> API: `https://<resource-name>.services.ai.azure.com/mai/v1/images/generations` | - **Input:** text <br /> - **Output:** One image. Maximum 1024×1024 pixels <br /> - **Context length**: 32,000 tokens <br /> - **Tool calling:** No <br /> - **Response formats:** Image (PNG) <br /> - **Key features:** Photorealistic and cinematic quality; production-grade output for logos, marketing, and design <br /> - **Parameters:** `width`, `height`, `prompt` <br /> Supports image resolutions ranging from 256×256 to 1024×1024 pixels, with `width` and `height` configurable within this range depending on aspect ratio | - Global standard (all regions) |
