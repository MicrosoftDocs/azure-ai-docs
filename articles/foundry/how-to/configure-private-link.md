...
> [!NOTE]
> As an alternative to the `AMLMachineLearning` service tag for evaluations, add `*.dataproxy.{region}.api.azureml.ms` and `{region}.api.azureml.ms` to your firewall allow list. Use the region where the evaluation runs. For example, if a benchmark evaluation runs in East US, add `eastus.api.azureml.ms`.
