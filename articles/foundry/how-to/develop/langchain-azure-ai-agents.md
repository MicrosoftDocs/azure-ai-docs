...
loop approvals, and async operations.
...
Install the package and configure the endpoint and model deployment. Use the
...
Set your environment variables in Python:
...
:::image type="content" source="../media/langchain-azure-ai-agents/react-agent-no-tools.png" alt-text="Diagram of the agent graph for an agent without tools.":::
...
:::image type="content" source="../media/langchain-azure-ai-agents/react-agent-tools.png" alt-text="Diagram of the agent graph for an agent with tools.":::
...
> is why no further configuration is required. You can change that to use an
...
:::image type="content" source="../media/langchain-azure-ai-agents/react-agent-approval.png" alt-text="Diagram of the agent graph for an agent with approval flows implemented.":::
...
To view the traces, it's important to understand that there are *2 agents* here:

1. The Foundry agent, which is the backend of one of the nodes of the graph.
2. The entire LangGraph graph, which is composed the multiple nodes.
...
2. Navigate to the Azure Application Insights configured.
...
5. Select **View Traces with Agent Runs**. The side panel shows all the traces generated by agent runs.
	...
	:::image type="content" source="../media/langchain-azure-ai-agents/react-agent-azmon-runs.png" alt-text="Screenshot showing the Agents (Preview) section in Azure Monitor displaying multiple runs.":::

6. Select one of the traces. You should see the details.

	:::image type="content" source="../media/langchain-azure-ai-agents/react-agent-azmon-trace.png" alt-text="Screenshot showing the trace details of the selected run.":::
