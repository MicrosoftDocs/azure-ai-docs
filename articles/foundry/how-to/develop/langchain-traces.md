...
Use `langchain-azure-ai` integration package to emit OpenTelemetry traces from LangChain and
...
> control if content from messages and tool calls are recorded in the trace.
...
You can also sink to an specific Azure Application Insights by specifying the
...
result = workflow.invoke({"messages": [input_message]}, config)
pretty_print(result)
```
...
If your deployed your LangGraph or LangChain solution, you can register that deployment into [Foundry Control Plane](../../control-plane/overview.md)
...
1. Ensure you meet the requirements to use Foundry Control Plane custom agents capability:
...
7. The registration wizard appears. First, complete the details about the agent that you want to register.
    ...
    - **OpenTelemetry Agent ID**: the parameter `agent_id` that you have configured in the `AzureAIOpenTelemetryTracer` class.
    - **Project**: the project that you configured to sink traces to in the `AzureAIOpenTelemetryTracer` class.
    - **Agent name**: the name of the agent (it can be the same as the `agent_id`).

8. Invoke the agent ot make sure it has runs.
