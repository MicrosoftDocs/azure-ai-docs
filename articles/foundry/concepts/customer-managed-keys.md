...
| Hosted Agents | ❌ Not supported | ❌ Not supported | Hosted agent compute is ephemeral. State that the agent writes to the `$HOME` folder is backed up to Microsoft-managed storage and encrypted with Microsoft-managed keys. Unlike prompt agents, hosted agents do not support bring-your-own resources. The `$HOME` folder can't be disabled. For details about the compute stack and session lifecycle, see [CMK coverage across the compute stack](#cmk-coverage-across-the-compute-stack). |
