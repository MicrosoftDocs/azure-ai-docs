...
| Phase | Pattern |
|-------|---------|
| **Azure Government launch** | Each model launches per its own deployment type and region availability matrix. Microsoft sets the retirement date in Azure Government to match the Commercial Cloud date. You can find this date through the [Models API](/rest/api/aiservices/accountmanagement/models). |
...
Microsoft sets the retirement date for GA models programmatically at commercial launch to 18 months out - there's no separate "announcement." Legacy and Deprecated transitions follow the published timeline and are visible in real time via the [Models API](/rest/api/aiservices/accountmanagement/models).
