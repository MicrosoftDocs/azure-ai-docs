...
- A [Foundry resource](/azure/foundry-classic/openai/how-to/create-resource) in a region with available PTU quota
...
You can view the quota available to you in a region by visiting the **Quota** pane in the [!INCLUDE [foundry-link](../../includes/foundry-link.md)] **Operate** section and selecting the desired subscription and region. 
...
REST, ARM template, Bicep, and Terraform can also be used to create deployments. See the section on automating deployments in the [Managing Quota](/azure/foundry-classic/openai/how-to/quota?tabs=rest#automate-deployment) how-to guide and replace the `sku.name` with `GlobalProvisionedManaged`, `DataZoneProvisionedManaged`, or `ProvisionedManaged` rather than `Standard`.
