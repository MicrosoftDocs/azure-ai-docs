...
> [!NOTE]
> You do not need to deploy the supported LLMs for use with model router with the exception of the Claude models. To use model router with your Claude models, first deploy them from the model catalog. The deployments will get invoked by Model router if selected for routing.
...
- In the default `Balanced` mode, it considers all underlying models within a small quality range, for example 1-2% compared with the highest quality model for that prompt, and pick the most cost-effective model.
- When the `Cost` routing mode is selected, it considers a larger quality band, for example. 5-6% range compared with the highest quality model for that prompt, and choose the most cost-effective model. 
