...
description: Learn how Prompt Shields in Microsoft Foundry detect and block user prompt attacks and indirect document attacks on your model deployments and agents.
author: ssalgadodev
ms.author: ssalgado
manager: nitinme
...
Prompt Shields detect and block attempts to manipulate your model's behavior through adversarial inputs. The feature protects against two types of attacks:
...
- **Indirect document attacks** — Hidden instructions embedded in third-party content (documents, emails, web pages) that try to hijack the model session. Scanned at the **user input** and **tool response** intervention points.
...
- **Indirect document attacks** — Hidden instructions embedded in third-party content (documents, emails, web pages) that try to hijack the model session.
...
Spotlighting enhances protection against indirect document attacks. When enabled, the service transforms document content using base-64 encoding so the model treats it as less trustworthy than direct user and system prompts. This helps prevent the model from executing unintended commands found in third-party documents.

Spotlighting is turned off by default. You can enable it when [configuring guardrail controls](../../default/guardrails/how-to-create-guardrails.md) in the Foundry portal or through the REST API. Spotlighting is only available for models used via the Chat Completions API.
