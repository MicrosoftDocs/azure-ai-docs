...
<!-- Changed per agent feedback: replaced generalized efficiency claim with a concrete dataset-size example. -->
...
Fine-tuned models require shorter prompts because they are trained on relevant examples; for example, applications often reduce prompts from dozens of in-context examples to a single instruction, lowering per-request token counts and cost. 
<!-- Changed per agent feedback: tightened benefit statement and added a concrete prompt-length and token-cost example. -->
...
Fine-tuning is suited for times when you have a relatively small, high-quality dataset (for example, hundreds to a few thousand task-specific prompt–response pairs) and want to improve the performance of your model. Fine-tuning can be for different kinds of use cases - but they often fall into broader categories. 
<!-- Changed per agent feedback: replaced vague data-size wording with a concrete dataset-oriented example. -->
