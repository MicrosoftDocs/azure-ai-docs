...
The Foundry (new) portal displays only the Default project for each Foundry resource, not other resources or hub-based projects you might have created in Foundry (classic). If you have created multiple projects under the same Foundry resource, you can identify which project is the default by checking the Microsoft Foundry (classic) portal, where the default project is marked with (default) next to its name.
