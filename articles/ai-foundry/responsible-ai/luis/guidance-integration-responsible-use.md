...
* **Maintain security.** Implement encryption and role-based access control in the client application to preserve the integrity of your content and prevent unauthorized access.
<!-- Changed to specify concrete security controls and scope, per agent feedback -->
...
* **Always plan to have the user confirm an action before being processed.** Plan to have your user confirm an action before the client application or backend processes the request to avoid incorrect responses that might come from the LUIS application. For example, suppose your LUIS application is integrated in an HR bot and LUIS returns that your user requests five days off. Have the user confirm that this request is the correct action to be taken before processing it.
<!-- Changed to clarify processing scope and remove ambiguous phrasing, per agent feedback -->
