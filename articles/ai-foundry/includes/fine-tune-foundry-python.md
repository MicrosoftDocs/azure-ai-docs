...
The training and validation data you use **must** be formatted as a JSON Lines (JSONL) document and must be formatted in the conversational format that is used by the Chat completions API.

### Example file format

```json
[Code example placeholder - training data in JSONL format]
```
...
```json
[Code example placeholder - multi-turn conversation format]
```

### Chat completions with vision

```json
[Code example placeholder - vision-enabled chat format]
```
...
```python
[Code example placeholder - upload training files using Python SDK]
```
...
```python
[Code example placeholder - create fine-tuning job using Python SDK]
```

## Check your model status

You can check the status of the fine-tuning job and model by running the following code:

```python
[Code example placeholder - check fine-tuning job status]
```

## Deploy a custom model

Unlike the pre-trained base models, you cannot access your fine-tuned model without deploying it first to an endpoint. The following code shows how to deploy your fine-tuned model:

```python
[Code example placeholder - deploy fine-tuned model]
```

## Use a deployed customized model

After your fine-tuned model is deployed, you can use it like any other deployed model. For example, you can send a chat completion call to your deployed model, as shown in the following Python example.

```python
[Code example placeholder - use deployed fine-tuned model]
```
