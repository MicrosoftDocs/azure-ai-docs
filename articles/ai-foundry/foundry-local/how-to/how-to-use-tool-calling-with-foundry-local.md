...
Foundry Local can make use of tool calling, a technique that allows you to prompt the model with function or method specifications that together with the context provided in the user's prompt, allow the model to work out which functions should be called and with what input data. The application can then call those functions and further prompt the model with the results of those functions.
