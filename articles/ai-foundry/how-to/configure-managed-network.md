...
| `marketplace.visualstudio.com`<br>`vscode.blob.core.windows.net`<br>`*.gallerycdn.vsassets.io` | Required to download and install VS Code extensions. These hosts enable the remote connection to compute instances. Learn more in [Get started with Foundry projects in VS Code](./develop/get-started-projects-vs-code.md). |
