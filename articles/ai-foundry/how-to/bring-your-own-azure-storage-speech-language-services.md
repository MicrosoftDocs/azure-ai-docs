...
::: moniker range="ai-foundry-classic"
[!INCLUDE [feature-preview](../includes/feature-preview.md)]
::: moniker-end

::: moniker range="ai-foundry"
...
::: moniker range="ai-foundry"
Azure AI Foundry hubs act as an aggregator service, bringing together agents, speech, and language capabilities into a unified platform. This integration allows organizations to manage and deploy advanced AI services efficiently. Both Speech and Language services exist independently, each with their own methods for configuring storage and managing data. As a result, the interface and setup process for these services within Azure AI Foundry hubs might differ from what you experienced previously.
::: moniker-end
...
::: moniker range="ai-foundry"
This article shows you how to connect your Azure Storage account to your Azure AI Foundry hub for Speech and Language services. The limitations and instructions in this article apply only to Speech and Language services. To learn more about Azure AI Foundry's bring-your-own Azure Storage solutions for other features, see [Connect Azure Storage accounts to AI Foundry](bring-your-own-azure-storage-foundry.md).
::: moniker-end
...
::: moniker range="ai-foundry"
Before setting up your Azure Storage account with your Azure AI Foundry hub for Speech and Language services, ensure you have:

- An Azure subscription
- An Azure Storage account
- Contributor or Owner permissions on both the Azure AI Foundry hub and Storage account
- Understanding of the [restrictions](#understand-restrictions) for Speech and Language storage configuration
::: moniker-end
...
::: moniker range="ai-foundry"
- You can set only one Azure Storage account for Speech and Language capabilities.
- You must set the Azure Storage account during Azure AI Foundry hub creation. You can't set it after hub creation.
- You can't remove the Azure Storage account after you set it on the Azure AI Foundry hub.
- If you delete the Azure Storage account, Speech and Language services can't function. You need to create a new Azure AI Foundry hub if this issue occurs. This restriction also applies when you move the Azure Storage account to another Azure subscription. Changing the Storage account's resource ID results in access issues.
- The Azure Storage account is shared by both Speech and Language capabilities in the Azure AI Foundry hub. While Speech and Language capabilities write to different Azure storage containers, if your scenario requires strict data isolation, create separate Azure AI Foundry hubs and storage accounts.
::: moniker-end
...
::: moniker range="ai-foundry"
After setting up the Azure Storage account, grant the Azure AI Foundry hub proper permissions on the Azure Storage account so the service can access your data. The service supports only Azure role-based access control (RBAC) authentication.

Assign the Azure Storage Blob Data Contributor role on the Storage account for the Azure AI Foundry hub's managed identity. Set the role assignment at the Azure AI Foundry hub level, not for individual projects.
::: moniker-end
...
::: moniker range="ai-foundry"
You can associate Azure Storage accounts with your Azure AI Foundry hub for Speech and Language services by using infrastructure templates during hub creation.
::: moniker-end
...
1. Create an Azure AI Foundry hub with the `userOwnedStorage` field containing the Azure Storage account's resource ID.

1. Assign Storage Blob Data Contributor role for the Azure AI Foundry hub on the Storage account.
::: moniker-end
...
::: moniker range="ai-foundry"
When you create an Azure AI Foundry hub with Terraform AzureRM, use the storage block to pass in the Storage account's resource ID.

1. Review the [Terraform AzureRM cognitive account documentation](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/cognitive_account) to understand how to define the storage block.

1. Define the storage block in your `kind=AIServices` cognitive account, which represents the Azure AI Foundry hub.
...
::: moniker range="ai-foundry"
Speech services in an Azure AI Foundry hub generally follow the guidelines in [Bring your own storage (BYOS) Speech resource](https://learn.microsoft.com/azure/ai-services/speech-service/bring-your-own-storage-speech-resource?tabs=portal). This guidance applies to key Speech scenarios, including Speech-to-Text (batch and real-time), Custom Speech, Text-to-Speech, and Custom Voice. For specific features under Speech, see that documentation.
::: moniker-end
...
::: moniker range="ai-foundry"
When you attach your Azure Storage account, you can set your customer-managed keys for encryption of Speech data. If you don't associate your Azure Storage account, Speech services don't use the customer-managed keys set at the Azure AI Foundry hub level. All other services available through your Azure AI Foundry hub use the customer-managed key for encryption.
::: moniker-end
...
::: moniker range="ai-foundry"
The `userOwnedStorage` field in Azure AI Foundry hubs acts similarly to Language resources with one key difference: you can't update the Azure Storage account even if the Storage account is deleted. In standalone Language resources, you can update the storage account after deletion, but this capability isn't available in Azure AI Foundry hubs.
::: moniker-end
...
::: moniker range="ai-foundry"
Speech and Language services share the same storage account that you set on the Azure AI Foundry hub. You can't separate the data between users at the hub level. If a user has access to the Azure AI Foundry hub, they can access data from both Speech and Language scenarios. Both services use their own container naming conventions that keep each service separate on the Storage account.
::: moniker-end
