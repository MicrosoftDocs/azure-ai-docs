...
1. Clone the sample repository that contains the MCP server template:
...
For detailed implementation guidance, see the [Azure Functions MCP server sample documentation](https://github.com/Azure-Samples/remote-mcp-functions-python/tree/main?tab=readme-ov-file).
...
1. Enter your MCP server details:
   - **Endpoint**: Enter your remote MCP server endpoint URL (for example, `https://{function_app_name}.azurewebsites.net/runtime/webhooks/mcp`)
   - **Authentication**: Select the authentication method and provide required credentials
