...
#CustomerIntent: As a developer, I want  to ensure compliance and security of my assets within Azure AI Foundry so that I avoid any security and compliance issues.
---
...
> [!NOTE]
> Each section lists prerequisites for specific features.
...
1. On the **Policies** tab, review all applicable guardrail policies within your subscription and project. To expand the scope beyond a single project, adjust the project filter to **All projects** for an overview of the entire subscription. You can also switch subscriptions.
...
1. When you select a guardrail policy, refer to the right-hand side panel and select an asset to compare its guardrail settings with the requirements that the guardrail policy specifies.
...
1. In the **Policy Compliance** column, examine any assets marked as **Violation detected**. Select these rows to access further details. Assets might appear multiple times if they're subject to several guardrail policies.

1. In the right-hand panel that appears when you select an asset, review the governing guardrail policies and the specifics of any noncompliant guardrail policy.
...
1. Allow up to 30 minutes for the guardrail policy to appear in the Azure AI Foundry portal. Compliance results appear once Azure Policy completes its scan. The duration of the scan varies by scope size and resources.
...
1. Create Purview policies to govern Azure AI Foundry apps and agents in the [Microsoft Purview portal](https://purview.microsoft.com).
