...
- Integration with Microsoft Purview is currently available for calls made through the Azure AI Foundry inference endpoint (aka OpenAI-compatible chat completions API or /chat/completions endpoint). Every model deployed through Foundry's managed inference stack is covered by Purview.
