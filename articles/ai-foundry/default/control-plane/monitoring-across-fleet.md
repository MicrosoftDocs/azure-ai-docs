...
Control Plane discovers all the agents you have access to and uses the Azure Application Insights associated with the resources hosting your agent to help you monitor and diagnose your agents. This includes:
...
Because Control Plane aggregates information across resources within the subscription, different users may see different agents listed depending on their access. Control Plane requires agents to log diagnostic information following OpenTelemetry standard with semantic conventions for Generative AI applications. When such telemetry is available, Control Plane can:
...
Configuring Azure Application Insights on each resource hosting agents isn't mandatory but **it's strongly advisable**. Agents running on resource without Azure Application Insights don't have health metrics, cost tracking, or drill-down traces.
...
Once observability has been configured, ensure you have the following permissions:
