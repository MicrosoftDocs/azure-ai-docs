...
Use these sections to find available tools and example prompts for Azure AI Foundry MCP Server (preview). Use conversational prompts instead of API calls. 

## Dataset management

**evaluation_dataset_create (write)**: Create or update a dataset version in an Azure AI project.

- Upload my customer support Q&A dataset from this Azure Blob Storage URL.
- "Create a new dataset version 2.0 for my training data located at https://mystorageaccount.blob.core.windows.net/datasets/qa-dataset.jsonl" 
...
**evaluation_create (write)** - Create an evaluation run for a dataset using one or more evaluators 
...
**evaluation_get (read)** - List evaluation runs in the Azure AI Project 
...
**evaluation_comparison_create (write)** - Create comparison results of evaluations within a group 
...
**evaluation_comparison_get (read)** - Get or list comparison results of evaluations within a group 
...
**model_catalog_list (read)** - List models from the Azure AI Foundry Model Catalog 
...
**model_details_get (read)** - Get full model details and code sample from Azure AI Foundry 
...
**model_deploy (write)** - Create or update a model deployment in the specified Azure AI Foundry account 
...
**model_deployment_get (read)** - Get one or more model deployments from an Azure AI Foundry account 
...
- "Clean up that deprecated model deployment from my Foundry account X" 

## Model analytics and recommendations 

**model_benchmark_get (read)** - Fetch benchmark data for Azure AI Foundry models 
...
**model_similar_models_get (read)** - Returns a list of similar models based on deployment or model details 
...
**model_monitoring_metrics_get (read)** - Get monitoring metrics for a model deployment 
...
**model_deprecation_info_get (read)** - Get deployment info enriched with deprecation data 
