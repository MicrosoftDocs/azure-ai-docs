...
By default, `include_detailed_errors` is `False`. Function execution and
argument-validation failures return generic results to the model and other
serialized channels. The original diagnostic remains available to trusted host
code in `Content.exception`. `Content.to_dict()` replaces that field with a
fixed, non-sensitive failure marker, and protocol conversions don't expose the
diagnostic.

Set
`client.function_invocation_configuration["include_detailed_errors"] = True`
only when the result stays on a trusted channel. Exception text can contain
sensitive data, and this setting adds that text to the channel-visible result.
